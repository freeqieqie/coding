...
// 调度日志级别：verbose(详细) | summary(默认汇总) | off(仅错误)
const SCHEDULER_LOG_LEVEL = String(process.env.SCHEDULER_LOG_LEVEL || 'summary').toLowerCase();
function schedulerLogVerbose(message) {
    if (SCHEDULER_LOG_LEVEL === 'verbose') {
        console.log(message);
    }
}
function schedulerLogSummary(message) {
    if (SCHEDULER_LOG_LEVEL === 'summary' || SCHEDULER_LOG_LEVEL === 'verbose') {
        console.log(message);
    }
}
...
function clearQuarterlyData() {
    db.run('DELETE FROM records', function(err) {
        if (err) {
            ...
            schedulerLogSummary(`[季度清理] 已清空 records 表，删除记录数: ${this.changes}`);
        }
    });
}
...
function clearWeeklyData() {
    db.run('DELETE FROM king_unlocks', function(err) {
        if (err) {
            ...
            schedulerLogSummary('每周清理king_unlocks表成功');
        }
    });
}
    ...
    db.run(
        ...
        function(err) {
            if (err) {
                ...
                schedulerLogSummary('每日清理所有年级大师赛解锁成功');
            }
        }
    );
}
...
setInterval(() => {
    console.clear();
    schedulerLogSummary(`[${new Date().toLocaleString()}] 控制台已自动清空。`);
}, 6 * 60 * 60 * 1000); // 6小时
...
function cleanExpiredVocabulary() {
    schedulerLogVerbose('开始清理过期单词...');
    // 获取10950天前的日期
    const expiryDate = new Date();
    expiryDate.setDate(expiryDate.getDate() - 10950); // 30年 = 10950天
    const expiryDateStr = expiryDate.toISOString();
    
    // 仅获取映射表中登记过的真实用户词汇表，避免误清理系统/映射表
    vocabDb.all(`SELECT table_name FROM vocabustu_user_map`, [], (err, tables) => {
        if (err) {
        ...
        let cleanedCount = 0;
        let skippedNoMasteredAtCount = 0;
        let pendingTables = tables.length;
        
        if (pendingTables === 0) {
            schedulerLogSummary('词汇掌握库清理完成：没有找到用户表');
            return;
        }
        ...
        tables.forEach(table => {
            ...
            if (!tableName) {
                if (--pendingTables === 0) {
                    schedulerLogSummary(`词汇掌握库清理完成，共清理 ${cleanedCount} 个过期单词，跳过缺少字段表 ${skippedNoMasteredAtCount} 个`);
                }
                return;
            }
            // 安全处理表名
            ...
            // 先检查表结构，兼容历史表结构缺少 mastered_at 的情况
            vocabDb.all(`PRAGMA table_info(${safeTable})`, [], (schemaErr, columns) => {
                if (schemaErr) {
                    console.error(`读取表 ${tableName} 结构失败:`, schemaErr.message);
                    if (--pendingTables === 0) {
                        schedulerLogSummary(`词汇掌握库清理完成，共清理 ${cleanedCount} 个过期单词，跳过缺少字段表 ${skippedNoMasteredAtCount} 个`);
                    }
                    return;
                ...
                const hasMasteredAt = Array.isArray(columns) && columns.some(col => col.name === 'mastered_at');
                if (!hasMasteredAt) {
                    skippedNoMasteredAtCount += 1;
                    schedulerLogVerbose(`跳过表 ${tableName}：缺少 mastered_at 字段`);
                    if (--pendingTables === 0) {
                        schedulerLogSummary(`词汇掌握库清理完成，共清理 ${cleanedCount} 个过期单词，跳过缺少字段表 ${skippedNoMasteredAtCount} 个`);
                    }
                    return;
                ...
                vocabDb.run(
                    ...
                    function(deleteErr) {
                        if (deleteErr) {
                            console.error(`清理表 ${tableName} 中的过期单词失败:`, deleteErr.message);
                        } else {
                            cleanedCount += this.changes;
                            schedulerLogVerbose(`从表 ${tableName} 中清理了 ${this.changes} 个过期单词`);
                        }
...
function scheduleVocabularyCleanup() {
    ...
    schedulerLogSummary(`词汇掌握过期清理已设置，将在 ${new Date(now.getTime() + msUntilMidnight).toLocaleString()} 首次执行`);
}
...
// 工具：并发限流映射，避免大批量 Promise.all 导致瞬时内存峰值过高
...
app.get('/api/vocab-ranking-top', async (req, res) => {
    try {
        ...
        // 并发限流获取词汇量，避免用户量大时瞬时内存峰值过高
        const all = await mapLimit(users, 5, async (u) => {
...
app.post('/api/audio-temp-upload', tempAudioUpload.single('audio'), (req, res) => {
    ...
    res.json({ success: true, filename: req.file.filename });
        ...
        const XF_IAT_ACCOUNTS = [
            // 这是13360328310的账号app01，已确认可用
            {
                appId: "74a8ba44",
                apiSecret: "MGY1YjNhM2U1ZWM4ZWJmMTcwODE0NmEx",
                apiKey: "d825216a1f5d095c7a53c6380f210a64",
                label: "账号1"
            },
            // 这是13360328310的账号app02，已确认可用
            {
                appId: "40648061",
                apiSecret: "MDg0YjE4ZWFjY2Y4NzRkN2IzNzcyZTcy",
                apiKey: "c37a717f2fa89f799b16682a7ee02019",
                label: "账号2"
            },
            // 这是13360328310的账号app03，已确认可用
            {
                appId: "ce2f174f",
                apiSecret: "MjI2MDhmNmY3ZDVlNDk4N2Y1MTczMzll",
                apiKey: "ac34f455d812e2693cc4af80cacf04c3",
                label: "账号3"
            },
            // 这是宇宇的手机号注册的账号，已确认可用
            {
                appId: "d5fe700f",
                apiSecret: "ZTQ0YzYxZThkZTNlZGZhMDFiZWZiMmJi",
                apiKey: "3c8066ebd3bf6f74df5ac577c71ed4ab",
                label: "账号4"
            },
            // 这是15815988017的账号，已确认可用
            {
                appId: "59c90050",
                apiSecret: "f04221c8438c75b6240dc81d339256ec",
                apiKey: "e935be78af1e5d4a8d6eb820d8d79746",
                label: "账号5"
            },
            // 这是13360328310的账号app06，已确认可用
            {
                appId: "7199cd58",
                apiSecret: "Njg2MGYxMzY4ODk0NmVjNWJhNDgyZjJi",
                apiKey: "1578b32c9ffbc2b9c4611a63f32449e7",
                label: "账号6"
            },
            // 这是13360328310的账号app07，已确认可用
            {
                appId: "c692afc2",
                apiSecret: "ZWRiYjMxNTZkM2E3NjgyMTc3ZGIyNTgz",
                apiKey: "26df015ef8a2bf3fe0afdf7a46b3da78",
                label: "账号7"
            }
        ];
...
app.get('/api/xfyun-signurl', (req, res) => {
        const requestedIndex = Number.parseInt(req.query.accountIndex, 10);
        const accountIndex = Number.isInteger(requestedIndex) && requestedIndex >= 0 ? requestedIndex : 0;
        const account = XF_IAT_ACCOUNTS[accountIndex];

        if (!account) {
            return res.status(404).json({ success: false, error: '未找到可用的讯飞签名账号' });
        }

        const date = new Date().toUTCString();
        const requestLine = 'GET /v2/iat HTTP/1.1';
        const signatureOrigin = `host: ${HOST}\ndate: ${date}\n${requestLine}`;
        const hmac = crypto.createHmac('sha256', account.apiSecret);
        hmac.update(signatureOrigin);
        const signature = hmac.digest('base64');
        const authorizationOrigin = `api_key="${account.apiKey}", algorithm="hmac-sha256", headers="host date request-line", signature="${signature}"`;
        ...
        res.json({
            ...
            appId: account.appId,
            accountIndex,
            accountCount: XF_IAT_ACCOUNTS.length,
            label: account.label
        });
    });
...
function scheduleOralListeningWeeklyClear() {
    const now = Date.now();
    const nextMonday = getNextMondayMidnight();
    let msUntilNextMonday = nextMonday - now;
    if (msUntilNextMonday <= 0) {
        msUntilNextMonday += 7 * 24 * 3600 * 1000;
    }
    setTimeout(() => {
        db.run('DELETE FROM oral_listening_progress', function(err) {
            if (err) {
                ...
                schedulerLogSummary('口语听力训练记录已清零');
            }
        });
        // 之后每周清理一次
        setInterval(() => {
...
db.serialize(() => {
    ...
    db.run(`ALTER TABLE messages ADD COLUMN quoted_message TEXT`, err => {
        if (err && !/duplicate column name/i.test(String(err.message || ''))) {
            console.error('messages 表新增 quoted_message 字段失败:', err);
        }
    });
...
function normalizeQuotedMessagePayload(input) {
...
app.post('/api/messages', (req, res) => {
    const { category, author, content, timestamp, quotedMessage } = req.body;
    ...
    const now = new Date().toISOString();
    const normalizedQuotedMessage = normalizeQuotedMessagePayload(quotedMessage);
    const quotedMessageValue = normalizedQuotedMessage ? JSON.stringify(normalizedQuotedMessage) : null;
    
    db.run(
        `INSERT INTO messages (category, author, content, quoted_message, timestamp, created_at) 
         VALUES (?, ?, ?, ?, ?, ?)`,
        [category, author, content, quotedMessageValue, timestamp, now],
        function(err) {
            if (err) {
...
app.get('/api/messages', (req, res) => {
    const { category, limit, offset = 0 } = req.query;
    ...
    let sql = `SELECT id, category, author, content, quoted_message, timestamp, created_at
         FROM messages 
         WHERE category = ?
    ...
    if (Number.isFinite(parsedLimit) && parsedLimit > 0) {
        sql += `
         LIMIT ? OFFSET ?`;
        params.push(parsedLimit, offsetNum);
    } else if (offsetNum > 0) {
        sql += `
         LIMIT -1 OFFSET ?`;
    ...
    db.all(sql, params, (err, rows) => {
        ...
        const normalizedRows = (rows || []).map(row => {
            ...
            if (row && row.quoted_message) {
                try {
                    quotedMessage = JSON.parse(row.quoted_message);
                } catch (error) {
                    quotedMessage = null;
                }
            }
...
app.post('/api/messages/recall', (req, res) => {
    ...
    db.get(
        ...
        (selectErr, row) => {
            if (selectErr) {
                console.error('查询留言失败:', selectErr);
            ...
            if (String(row.author).trim() !== String(username).trim()) {
                return res.status(403).json({ error: '只能撤回自己发布的留言' });
            }
            ...
            db.run(
                ...
                function(deleteErr) {
                    if (deleteErr) {
                        console.error('撤回留言失败:', deleteErr);
                        return res.status(500).json({ error: '撤回留言失败' });
                    }

                    if (this.changes === 0) {
                        return res.status(404).json({ error: '留言不存在或已被删除' });
                    }

                    res.json({ success: true, message: '留言撤回成功' });
                }
            );
        }
    );
...
// 4. 删除训练记录接口（支持按用户名、年级、单元筛选；不传参数则清空全部）
app.post('/api/prerev/records/clear', (req, res) => {
    const { username, grade, unit } = req.body || {};
    const where = [];
    const params = [];
    ...
    const sql = where.length
        ? `DELETE FROM prerev_record WHERE ${where.join(' AND ')}`
        : `DELETE FROM prerev_record`;
    ...
    db.run(sql, params, function(err) {
        ...
        res.json({
            ...
            scope: { username: username || null, grade: grade || null, unit: unit || null }
        });
    });
});
// 预习课文单词训练记录模块结束
...
function cleanTaskAudioDir() {
    fs.readdir(taskAudioDir, (err, files) => {
        ...
        if (files.length > 0) {
            schedulerLogSummary(`[taskaudio清理] 已删除 ${deleted}/${files.length} 个音频文件`);
        }
    });
}
    ...
    db.run(
        ...
        function(err) {
            if (err) {
                ...
                schedulerLogSummary(`[口语训练清理] 已清理30天前口语训练记录: ${this.changes}`);
            }
        }
    );
}
    ...
    try {
        ...
        const rows = await dbAll(
            ...
            SELECT
                ...
                sr.biscuit_count AS biscuit_count
        ...
        const students = rows
            ...
            .map(row => {
                ...
                return {
                    ...
                    biscuit_count: Number(row.biscuit_count || 0)
                };
            });
    ...
    try {
        ...
        try {
            const reward = await dbGet(
                `SELECT pet_food, biscuit_count FROM student_rewards WHERE username = ? AND source = ?`,
                [username, account.source]
            );
            if (reward) {
                petFoodCount = Number(reward.pet_food || 0);
                biscuitCount = Number(reward.biscuit_count || 0);
            }
        } catch (_) { /* 表可能不存在，忽略 */ }
...
// 宠物等级成长值上限计算: 每级需要 (level + 1) * 40 的成长值
// 也就是说：0 -> 1 需要 40，1 -> 2 需要 80，2 -> 3 需要 120，以此类推
function getPetMaxGrowth(level) {
    const lv = Math.min(Math.max(Number(level) || 0, 0), PET_MAX_LEVEL);
    return (lv + 1) * 40;
}

function applyPetGrowthDelta(level, growthValue, delta) {
    let nextLevel = Math.min(Math.max(Number(level) || 0, 0), PET_MAX_LEVEL);
    let nextGrowth = Math.max(Number(growthValue) || 0, 0) + delta;
    let maxGrowth = getPetMaxGrowth(nextLevel);

    while (nextLevel < PET_MAX_LEVEL && nextGrowth >= maxGrowth) {
    ...
    if (nextLevel >= PET_MAX_LEVEL) {
        nextGrowth = Math.min(nextGrowth, getPetMaxGrowth(PET_MAX_LEVEL));
    }
...
function applyTimedPetAutoChanges(level, growthValue, satiety, elapsedHours) {
    ...
    while (remainingHours > 1e-9) {
        ...
        if (!satietyStage.rate) {
            totalGrowthDelta += remainingHours * growthRate;
            remainingHours = 0;
            break;
        }
...
db.serialize(() => {
    ...
    db.run(`UPDATE pet_garden_pet_records SET level = 9 WHERE level > 9`, () => {});
    db.run(`UPDATE pet_garden_pets SET level = 9 WHERE level > 9`, () => {});
});

async function ensurePetGardenStudentClassNullable() {
    try {
        const columns = await dbAll(`PRAGMA table_info(pet_garden_students)`);
        if (!columns.length) return;

        const classIdColumn = columns.find(column => column.name === 'class_id');
        if (!classIdColumn || Number(classIdColumn.notnull || 0) !== 1) return;

        const hasCurrentPetId = columns.some(column => column.name === 'current_pet_id');
        await dbRun(`PRAGMA foreign_keys = OFF`);
        try {
            await dbRun(`BEGIN TRANSACTION`);
            await dbRun(`CREATE TABLE pet_garden_students_new (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                class_id INTEGER DEFAULT NULL,
                name TEXT NOT NULL,
                student_number TEXT DEFAULT NULL,
                ...
                createdAt TEXT NOT NULL,
                updatedAt TEXT NOT NULL,
                current_pet_id INTEGER DEFAULT NULL,
                FOREIGN KEY(class_id) REFERENCES pet_garden_classes(id)
            )`);
            await dbRun(
                `INSERT INTO pet_garden_students_new (
                    id, class_id, name, student_number, sort_order, linked_username, linked_type, createdAt, updatedAt, current_pet_id
                )
                SELECT
                    id, class_id, name, student_number, sort_order, linked_username, linked_type, createdAt, updatedAt,
                    ${hasCurrentPetId ? 'current_pet_id' : 'NULL'}
                FROM pet_garden_students`
            );
            await dbRun(`DROP TABLE pet_garden_students`);
            await dbRun(`ALTER TABLE pet_garden_students_new RENAME TO pet_garden_students`);
            await dbRun(`COMMIT`);
        } catch (err) {
            try {
                await dbRun(`ROLLBACK`);
            } catch (rollbackErr) {
                console.error('[petgarden] 回滚 pet_garden_students 迁移失败:', rollbackErr.message);
            }
            throw err;
        } finally {
        ...
        console.log('[petgarden] pet_garden_students.class_id 已迁移为可空');
    } catch (err) {
        console.error('[petgarden] 迁移 pet_garden_students.class_id 失败:', err.message);
    }
}

const petGardenStudentSchemaReady = ensurePetGardenStudentClassNullable();
...
function canAddExtraPet(pets) {
    return Array.isArray(pets) && pets.length > 0 && pets.every(pet => Number(pet.level || 0) >= 8);
}

async function listPetGardenStudentsByLinkedAccount(linkedUsername, linkedType) {
    return dbAll(
        `SELECT id, class_id, name, student_number, sort_order, current_pet_id, createdAt, updatedAt
         FROM pet_garden_students
         WHERE linked_username = ? AND linked_type = ?
         ORDER BY CASE WHEN class_id IS NULL THEN 1 ELSE 0 END ASC, datetime(updatedAt) DESC, id DESC`,
        [linkedUsername, linkedType]
    );
}

async function getPrimaryPetGardenStudentByLinkedAccount(linkedUsername, linkedType) {
    const rows = await listPetGardenStudentsByLinkedAccount(linkedUsername, linkedType);
    return rows[0] || null;
}

async function getReusableUnassignedPetGardenStudent(linkedUsername, linkedType) {
    return dbGet(
        `SELECT id, class_id, name, student_number, sort_order, current_pet_id, createdAt, updatedAt
         FROM pet_garden_students
         WHERE linked_username = ? AND linked_type = ? AND class_id IS NULL
         ORDER BY datetime(updatedAt) DESC, id DESC
         LIMIT 1`,
        [linkedUsername, linkedType]
    );
...
async function ensurePetGardenStudentHasPetRecord(studentId, nowIso = new Date().toISOString()) {
    const firstPet = await dbGet(`SELECT id FROM pet_garden_pet_records WHERE student_id = ? ORDER BY id ASC LIMIT 1`, [studentId]);
    if (!firstPet) {
        return createPetGardenEggRecord(studentId, nowIso);
    }
    return ensureStudentCurrentPetId(studentId);
}
...
petGardenStudentSchemaReady
    .catch(() => {})
    .then(() => migratePetGardenPetData());
...
app.post('/api/petgarden/student/add', async (req, res) => {
    ...
    try {
        ...
        const reusable = await getReusableUnassignedPetGardenStudent(linked_username, linked_type);
        if (reusable) {
            await dbRun(
                `UPDATE pet_garden_students
                 SET class_id = ?, name = ?, student_number = ?, sort_order = ?, updatedAt = ?
                 WHERE id = ?`,
                [class_id, name, student_number ? student_number.trim() : null, nextSort, now, reusable.id]
            );
            await ensurePetGardenStudentHasPetRecord(reusable.id, now);
            return res.json({ success: true, studentId: reusable.id, name, reused: true });
        }
...
app.post('/api/petgarden/student/batch-add', async (req, res) => {
    ...
    try {
        ...
        for (const stu of students) {
            ...
            const reusable = await getReusableUnassignedPetGardenStudent(stu.username, stu.type);
            if (reusable) {
                await dbRun(
                    `UPDATE pet_garden_students
                     SET class_id = ?, name = ?, student_number = ?, sort_order = ?, updatedAt = ?
                     WHERE id = ?`,
                    [class_id, stu.username, stu.student_number ? stu.student_number.trim() : null, nextSort++, now, reusable.id]
                );
                await ensurePetGardenStudentHasPetRecord(reusable.id, now);
                added.push({ studentId: reusable.id, username: stu.username, type: stu.type, reused: true });
                continue;
            }
...
app.post('/api/petgarden/student/ensure-for-account', async (req, res) => {
    ...
    try {
        ...
        // 已存在则直接返回
        const existing = await getPrimaryPetGardenStudentByLinkedAccount(linked_username, linked_type);
        if (existing) {
            await ensurePetGardenStudentHasPetRecord(existing.id);
            return res.json({ success: true, studentId: existing.id, created: false });
        }
...
app.post('/api/petgarden/pet/add-pet', async (req, res) => {
    ...
    try {
        ...
        if (!canAddExtraPet(pets)) {
            return res.status(400).json({ error: '只有当该学生现有的所有宠物都达到 8 级及以上时，才可以添加新宠物' });
        }
        const now = new Date().toISOString();
        const result = await dbRun(
            `INSERT INTO pet_garden_pet_records (student_id, pet_type, pet_name, level, growth_value, satiety, satiety_updated_at, adopted_at, createdAt, updatedAt)
...
app.post('/api/petgarden/pet/set-data', async (req, res) => {
    ...
    try {
        const pet = await getStudentTargetPetRecord(student_id, pet_id || null);
        if (!pet) return res.status(404).json({ error: '未找到宠物记录' });
        const targetLevel = (level != null && Number(level) >= 0) ? Number(level) : Number(pet.level || 0);
        const newLevel = Math.min(PET_MAX_LEVEL, Math.max(0, Math.floor(targetLevel)));
        const targetGrowth = (growth_value != null && Number(growth_value) >= 0) ? Number(growth_value) : Number(pet.growth_value || 0);
        const newGrowth = Math.min(getPetMaxGrowth(newLevel), Math.max(0, targetGrowth));
...
app.get('/api/petgarden/ranking', async (req, res) => {
    try {
        const rows = await dbAll(
            `SELECT r.id AS pet_record_id, r.student_id, r.pet_type, r.pet_name,
                    ...
                    s.name AS student_name, s.linked_username, s.linked_type,
                          ...
                          COALESCE(sr.biscuit_count, 0) AS biscuit_count
        ...
        const list = rows.map(row => {
            ...
            return {
                // include linked username so front-end can call reward APIs
...
app.get('/api/petgarden/my-pets', async (req, res) => {
    ...
    try {
        ...
        const students = source
            ? await dbAll(
                `SELECT id, name, class_id, current_pet_id
                 FROM pet_garden_students
                 WHERE linked_username = ? AND linked_type = ?
                 ORDER BY CASE WHEN class_id IS NULL THEN 1 ELSE 0 END ASC, datetime(updatedAt) DESC, id DESC`,
                [username, source]
...
db.serialize(() => {
    db.run(`CREATE TABLE IF NOT EXISTS student_rewards (
        ...
        biscuit_count INTEGER NOT NULL DEFAULT 0,
        updatedAt TEXT NOT NULL,
        UNIQUE(username, source)
    )`);

    db.run(`ALTER TABLE student_rewards ADD COLUMN biscuit_count INTEGER DEFAULT 0`, () => {});
    ...
    db.run(`CREATE TABLE IF NOT EXISTS pet_auto_feed_config (
        ...
        feed_type TEXT NOT NULL DEFAULT 'pet_food',
        enabled INTEGER NOT NULL DEFAULT 1,
        createdAt TEXT NOT NULL,
        updatedAt TEXT NOT NULL
    )`);

    db.run(`ALTER TABLE pet_auto_feed_config ADD COLUMN feed_type TEXT DEFAULT 'pet_food'`, () => {});
...
const FEED_ITEM_CONFIG = {
    candy:    { satietyDelta: 2,  label: '糖果', weeklyLimit: 100 },
    pet_food: { satietyDelta: 10, label: '宠物主食', weeklyLimit: 40 },
    biscuit:  { satietyDelta: 5,  label: '饼干', weeklyLimit: 40 }
};

const AUTO_FEED_ITEM_TYPES = ['pet_food', 'biscuit'];
...
async function getOrCreateStudentReward(username, source) {
    ...
    const now = new Date().toISOString();
    await dbRun(
        `INSERT INTO student_rewards (username, source, pet_food, biscuit_count, updatedAt) VALUES (?, ?, 0, 0, ?)`,
        [username, source, now]
    );
    return dbGet(
        `SELECT * FROM student_rewards WHERE username = ? AND source = ?`,
...
app.post('/api/gameself/add-pet-food', async (req, res) => {
    ...
    try {
        ...
        const petGardenStudent = await getPrimaryPetGardenStudentByLinkedAccount(username, source);
        if (!petGardenStudent) {
...
// API：增加饼干（训练页面完成后调用）
app.post('/api/gameself/add-biscuit', async (req, res) => {
    const username = String(req.body.username || '').trim();
    const amount = Math.floor(Number(req.body.amount));
    if (!username || !Number.isFinite(amount) || amount <= 0) {
        return res.status(400).json({ error: '参数无效' });
    ...
    try {
        ...
        const reward = await getOrCreateStudentReward(username, source);
        const newCount = Number(reward.biscuit_count || 0) + amount;
        const now = new Date().toISOString();
        await dbRun(
            `UPDATE student_rewards SET biscuit_count = ?, updatedAt = ? WHERE id = ?`,
            [newCount, now, reward.id]
        );
        res.json({ success: true, rewarded: true, biscuit_count: newCount });
    } catch (err) {
        res.status(500).json({ error: '增加饼干失败: ' + err.message });
    }
});

// API：增加糖果（训练页面完成后调用）
app.post('/api/gameself/add-candy', async (req, res) => {
    const username = String(req.body.username || '').trim();
    const amount = Math.floor(Number(req.body.amount));
    if (!username || !Number.isFinite(amount) || amount <= 0) {
        return res.status(400).json({ error: '参数无效' });
    ...
    try {
        ...
        const targetStudent = await dbGet(
            `SELECT id, candies_json
             FROM choose_game_students
             WHERE linked_username = ? AND linked_type = ?
             ORDER BY datetime(updatedAt) DESC, id DESC
        ...
        if (!targetStudent) {
            return res.json({ success: true, rewarded: false, reason: 'student_not_in_choose_game' });
        }

        const currentCandies = parseChooseCandiesJson(targetStudent.candies_json);
        const appendedCandies = Array.from({ length: amount }, (_, index) => ({
            id: `reward_${Date.now()}_${index}_${Math.random().toString(36).slice(2, 8)}`,
            mode: 'scatter',
            left: null,
            top: null,
            rotation: 0,
        ...
        const nextCandies = currentCandies.concat(appendedCandies);
        const now = new Date().toISOString();
        await dbRun(
            `UPDATE choose_game_students SET candies_json = ?, updatedAt = ? WHERE id = ?`,
            [JSON.stringify(nextCandies), now, targetStudent.id]
        );
        res.json({ success: true, rewarded: true, candy_count: nextCandies.length });
    } catch (err) {
        res.status(500).json({ error: '增加糖果失败: ' + err.message });
    }
});
...
// API：减少饼干（老师/管理端收回饼干时调用）
app.post('/api/gameself/remove-biscuit', async (req, res) => {
    const username = String(req.body.username || '').trim();
    const amount = Math.floor(Number(req.body.amount));
    if (!username || !Number.isFinite(amount) || amount <= 0) {
        return res.status(400).json({ error: '参数无效' });
    ...
    try {
        ...
        const reward = await getOrCreateStudentReward(username, source);
        if ((reward.biscuit_count || 0) < amount) {
            return res.json({ success: true, removed: false, reason: 'not_enough', biscuit_count: reward.biscuit_count || 0 });
        }

        const newCount = Math.max(0, (reward.biscuit_count || 0) - amount);
        const now = new Date().toISOString();
        await dbRun(
            `UPDATE student_rewards SET biscuit_count = ?, updatedAt = ? WHERE id = ?`,
            [newCount, now, reward.id]
        );
        res.json({ success: true, removed: true, biscuit_count: newCount });
    } catch (err) {
        res.status(500).json({ error: '减少饼干失败: ' + err.message });
    }
});
...
// API：学生喂食宠物（消耗糖果、宠物主食或饼干来增加宠物饱腹度）
// feedType: 'candy' | 'pet_food' | 'biscuit'
// quantity: 使用数量
app.post('/api/gameself/feed-pet', async (req, res) => {
    ...
    try {
        ...
        selectedFeedTypes.forEach(feedType => {
            ...
            if (feedType === 'biscuit' && requested > inventory.biscuitCount) {
                throw createHttpError(400, `饼干不足，需要 ${requested} 个，当前只有 ${inventory.biscuitCount} 个`);
            }
        });
        ...
        try {
            ...
            if (quantities.biscuit > 0) {
                const reward = await getOrCreateStudentReward(username, account.source);
                const now = new Date().toISOString();
                await dbRun(
                    `UPDATE student_rewards SET biscuit_count = biscuit_count - ?, updatedAt = ? WHERE id = ?`,
                    [quantities.biscuit, now, reward.id]
                );
            }
...
// 自动喂食日志级别：verbose(逐条明细) | summary(默认，仅汇总) | off(仅错误)
const AUTO_FEED_LOG_LEVEL = String(process.env.AUTO_FEED_LOG_LEVEL || 'summary').toLowerCase();

function isAutoFeedVerbose() {
    return AUTO_FEED_LOG_LEVEL === 'verbose';
}

function shouldAutoFeedLogSummary() {
    return AUTO_FEED_LOG_LEVEL === 'summary' || AUTO_FEED_LOG_LEVEL === 'verbose';
}

function autoFeedLogVerbose(message) {
    if (isAutoFeedVerbose()) {
        console.log(message);
    }
}

function autoFeedLogSummary(message) {
    if (shouldAutoFeedLogSummary()) {
        console.log(message);
    }
}
...
async function feedPetInternal(petId, username, source, quantities) {
    ...
    for (const feedType of selectedFeedTypes) {
        ...
        if (feedType === 'biscuit' && requested > inventory.biscuitCount) {
            throw new Error(`饼干不足（需要 ${requested}，当前 ${inventory.biscuitCount}）`);
        }
    }
    ...
    try {
        ...
        const petRows = await dbAll(
            `SELECT p.id, p.pet_type, p.pet_name, p.satiety, p.level,
                    c.threshold, c.feed_type, c.enabled
             FROM pet_garden_pet_records p
        ...
        const pets = petRows.map(row => {
            ...
            return {
                ...
                feedType: AUTO_FEED_ITEM_TYPES.includes(row.feed_type) ? row.feed_type : 'pet_food',
...
app.post('/api/gameself/auto-feed-config/save', async (req, res) => {
    ...
    try {
        ...
        for (const cfg of configs) {
            ...
            const feedType = AUTO_FEED_ITEM_TYPES.includes(String(cfg.feedType || '').trim())
                ? String(cfg.feedType || '').trim()
                : 'pet_food';
            const enabled = cfg.enabled ? 1 : 0;
            await dbRun(
                `INSERT INTO pet_auto_feed_config (pet_record_id, threshold, feed_type, enabled, createdAt, updatedAt)
                 VALUES (?, ?, ?, ?, ?, ?)
                 ON CONFLICT(pet_record_id)
                 DO UPDATE SET threshold = excluded.threshold, feed_type = excluded.feed_type, enabled = excluded.enabled, updatedAt = excluded.updatedAt`,
                [petId, threshold, feedType, enabled, now, now]
            );
        }
...
async function runAutoFeedJob() {
    // 先找出所有开启自动喂食的账号和学生，再在后台同步宠物状态，避免依赖前端访问才刷新饱腹值。
    const autoFeedUsers = await dbAll(
        `SELECT DISTINCT s.id AS student_id, s.linked_username, s.linked_type
    ...
    if (!autoFeedUsers.length) return;

    const byUser = {};
    for (const row of autoFeedUsers) {
        const key = `${row.linked_username}|${row.linked_type}`;
        if (!byUser[key]) {
            byUser[key] = {
                username: row.linked_username,
                source: row.linked_type,
                studentIds: new Set()
            };
        }
        byUser[key].studentIds.add(row.student_id);
    }
    ...
    for (const userEntry of Object.values(byUser)) {
        const { username, source } = userEntry;
        runStats.users += 1;
        try {
            const userStats = {
                plans: 0,
                ...
                fedPetFood: 0,
                fedBiscuit: 0,
                skippedByNoQuotaOrInventory: 0,
                deathEvents: 0,
                errors: 0,
                skippedUserReason: ''
            };

            for (const studentId of userEntry.studentIds) {
                const syncResult = await syncStudentPetRecords(studentId, { withEvents: true });
                if (Array.isArray(syncResult?.death_events) && syncResult.death_events.length > 0) {
                    userStats.deathEvents += syncResult.death_events.length;
                    runStats.deathEvents += syncResult.death_events.length;
                    syncResult.death_events.forEach(event => {
                        autoFeedLogVerbose(`[AutoFeed] ${event.message}`);
                    });
                }
            }

            const configs = await dbAll(
                `SELECT c.pet_record_id, c.threshold, c.feed_type,
                            p.satiety, p.pet_type, p.pet_name
                 FROM pet_auto_feed_config c
                 INNER JOIN pet_garden_pet_records p ON p.id = c.pet_record_id
                 INNER JOIN pet_garden_students s ON s.id = p.student_id
                 WHERE c.enabled = 1
                   AND s.linked_username = ?
                   AND s.linked_type = ?
            ...
            for (const cfg of configs) {
                ...
                const feedType = AUTO_FEED_ITEM_TYPES.includes(cfg.feed_type) ? cfg.feed_type : 'pet_food';
                const feedConfig = FEED_ITEM_CONFIG[feedType];
                if (satiety < cfg.threshold) {
                    const needed = Math.ceil((cfg.threshold - satiety) / feedConfig.satietyDelta);
                    plans.push({
                        petId: cfg.pet_record_id,
            ...
            userStats.plans = plans.length;
            runStats.usersWithPlans += 1;

            const inventory = await getGameselfInventoryCounts(username, source);
            const inventoryByFeedType = {
                pet_food: Number(inventory.petFoodCount || 0),
                biscuit: Number(inventory.biscuitCount || 0)
            };

            let skipUser = false;
            for (const feedType of AUTO_FEED_ITEM_TYPES) {
                const planCount = plans.filter(plan => plan.feedType === feedType).length;
                if (planCount > 0 && inventoryByFeedType[feedType] < planCount) {
                    userStats.skippedUserReason = `${FEED_ITEM_CONFIG[feedType].label}不足（需至少 ${planCount}，有 ${inventoryByFeedType[feedType]}）`;
                    skipUser = true;
                    break;
                }
            }
            if (skipUser) {
                ...
                autoFeedLogVerbose(`[AutoFeed] ${username} 汇总: 待喂${userStats.plans}只，整用户跳过，原因=${userStats.skippedUserReason}`);
                continue;
            }

            const remainingByFeedType = { ...inventoryByFeedType };
            for (const plan of plans) {
                const feedType = plan.feedType;
                // 获取该宠物本周剩余配额，防止把超额请求传给 feedPetInternal 后触发异常
                let quotaRemaining = 0;
                try {
                    const weeklyQuota = await getPetWeeklyFeedUsageSummary(plan.petId);
                    quotaRemaining = (weeklyQuota[feedType] && weeklyQuota[feedType].remaining) || 0;
                } catch (qErr) {
                    userStats.errors += 1;
                    runStats.errors += 1;
                    console.error(`[AutoFeed] ${username} 宠物#${plan.petId} 获取配额失败: ${qErr.message}`);
                    continue;
                }
                // 同时受库存、配额三者共同限制（原代码漏了配额）
                const toFeed = Math.min(plan.needed, remainingByFeedType[feedType], quotaRemaining);
                if (toFeed <= 0) {
                    userStats.skippedByNoQuotaOrInventory += 1;
                    continue;
                } // 修复：原为 break，导致后续宠物全部跳过
                try {
                    const quantities = Object.keys(FEED_ITEM_CONFIG).reduce((acc, key) => {
                        acc[key] = key === feedType ? toFeed : 0;
                        return acc;
                    }, {});
                    const result = await feedPetInternal(plan.petId, username, source, quantities);
                    autoFeedLogVerbose(`[AutoFeed] ${username} 宠物#${plan.petId}: 喂食 ${toFeed} ${FEED_ITEM_CONFIG[feedType].label}，饱腹 ${plan.satiety} → ${result.newSatiety}`);
                    remainingByFeedType[feedType] -= toFeed;
                    userStats.fedPets += 1;
                    runStats.fedPets += 1;
                    if (feedType === 'pet_food') {
                        userStats.fedPetFood += toFeed;
                        runStats.fedPetFood += toFeed;
                    }
                    if (feedType === 'biscuit') {
                        userStats.fedBiscuit += toFeed;
                        runStats.fedBiscuit += toFeed;
                    }
                    // 写入喂食日志，保留最近10条
            ...
            if (
                userStats.fedPets > 0 ||
                userStats.skippedByNoQuotaOrInventory > 0 ||
                userStats.deathEvents > 0 ||
                userStats.errors > 0
            ) {
                autoFeedLogVerbose(
                    `[AutoFeed] ${username} 汇总: 待喂${userStats.plans}只, 已喂${userStats.fedPets}只, 主食${userStats.fedPetFood}, 饼干${userStats.fedBiscuit}, 因库存/配额跳过${userStats.skippedByNoQuotaOrInventory}只, 死亡事件${userStats.deathEvents}, 错误${userStats.errors}`
                );
            }
        } catch (userErr) {
            runStats.errors += 1;
    ...
    autoFeedLogSummary(
        `[AutoFeed] 本轮汇总: 用户${runStats.users}, 有计划${runStats.usersWithPlans}, 跳过用户${runStats.skippedUsers}, 已喂宠物${runStats.fedPets}, 主食${runStats.fedPetFood}, 饼干${runStats.fedBiscuit}, 死亡事件${runStats.deathEvents}, 错误${runStats.errors}`
    );
}
...
// 新增：保存专项训练（语法/完型）学生提交记录表
db.serialize(() => {
    db.run(`CREATE TABLE IF NOT EXISTS focus_submission (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    db.run(`CREATE INDEX IF NOT EXISTS idx_focus_submission_passage ON focus_submission(passage_id)`);
    db.run(`CREATE INDEX IF NOT EXISTS idx_focus_submission_user ON focus_submission(username)`);
});

// 获取某题目中某个考点的备选项（会随机打乱顺序）
app.get('/api/focus/passage/options', async (req, res) => {
    try {
        const passageId = Number(req.query.passage_id || req.query.id);
        const num = Number(req.query.num);
        if (!passageId || !num) return res.status(400).json({ success: false, error: '缺少 passage_id 或 num 参数' });
        const row = await dbGet(`SELECT questions_json FROM passagequestion WHERE id = ?`, [passageId]);
        if (!row) return res.status(404).json({ success: false, error: '未找到题目' });
        const questions = safeParseJson(row.questions_json || '[]');
        let q = questions.find(it => Number(it.num) === Number(num));
        if (!q) {
            // 兼容按索引传入 num (从1开始)
            q = questions[Number(num) - 1] || null;
        }
        if (!q) return res.status(404).json({ success: false, error: '未找到对应考点' });
        const opts = [];
        if (q.correct) opts.push(String(q.correct));
        if (Array.isArray(q.distractors)) q.distractors.forEach(d => { if (d) opts.push(String(d)); });
        // 去重
        ...
        res.json({ success: true, options: uniq });
    } catch (err) {
        res.status(500).json({ success: false, error: err.message || '获取备选项失败' });
    }
});

// 学生提交专项训练答案（服务端计算对错并保存记录）
app.post('/api/focus/submit', async (req, res) => {
    try {
        const { passage_id, username, student_name, answers, duration_seconds, duration_text, submit_time } = req.body || {};
        if (!passage_id || !username || !Array.isArray(answers)) return res.status(400).json({ success: false, error: '缺少 passage_id / username / answers' });

        const row = await dbGet(`SELECT questions_json FROM passagequestion WHERE id = ?`, [passage_id]);
        if (!row) return res.status(404).json({ success: false, error: '题目不存在' });
        const questions = safeParseJson(row.questions_json || '[]');

        // 标准化前端提交的 answers 格式：[{num: 1, selected: 'word'}, ...]
        const norm = (answers || []).map(a => ({ num: Number(a.num), selected: (a.selected || '').toString() }));

        const total = questions.length;
        let correct = 0;
        questions.forEach(q => {
            const ua = norm.find(x => Number(x.num) === Number(q.num));
            if (ua && ua.selected && q.correct && ua.selected.trim() === String(q.correct).trim()) correct++;
        });

        const now = submit_time || new Date().toISOString();
        const inserted = await dbRun(
            `INSERT INTO focus_submission (passage_id, username, student_name, answers_json, correct_count, total_count, duration_seconds, duration_text, submit_time)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [passage_id, username, student_name || '', JSON.stringify(answers || []), correct, total, Number(duration_seconds) || null, duration_text || null, now]
        );

        res.json({ success: true, id: inserted.lastID, correct_count: correct, total_count: total });
    } catch (err) {
        console.error('保存专项训练提交失败:', err);
        res.status(500).json({ success: false, error: err.message || '提交保存失败' });
    }
});

// 获取某题目下所有学生的提交记录
app.get('/api/focus/submissions', async (req, res) => {
    try {
        const { passage_id } = req.query || {};
        if (!passage_id) return res.status(400).json({ success: false, error: '缺少 passage_id 参数' });
        const rows = await dbAll(`SELECT * FROM focus_submission WHERE passage_id = ? ORDER BY submit_time DESC`, [passage_id]);
        const out = (rows || []).map(r => ({ ...r, answers: safeParseJson(r.answers_json) }));
...
// 获取某一学生在可选题目上的提交记录（可选按 passage_id 过滤）
app.get('/api/focus/submissions/user', async (req, res) => {
    try {
        const { username, passage_id } = req.query || {};
        if (!username) return res.status(400).json({ success: false, error: '缺少 username 参数' });
        let sql = `SELECT * FROM focus_submission WHERE username = ?`;
...
// Scheduled pruning: keep top N submissions per (passage_id, username)
async function pruneFocusSubmissionsTopN(n = 10) {
    try {
        const combos = await dbAll(`SELECT DISTINCT passage_id, username FROM focus_submission`);
        for (const combo of combos || []) {
            const passageId = combo.passage_id;
            const username = combo.username;
            if (passageId == null || !username) continue;
            const keepRows = await dbAll(
                `SELECT id FROM focus_submission WHERE passage_id = ? AND username = ? ORDER BY correct_count DESC, COALESCE(duration_seconds, 999999999), submit_time ASC LIMIT ?`,
                [passageId, username, n]
            );
            const keepIds = (keepRows || []).map(r => r.id).filter(Boolean);
            if (keepIds.length) {
                const placeholders = keepIds.map(() => '?').join(',');
                await dbRun(
                    `DELETE FROM focus_submission WHERE passage_id = ? AND username = ? AND id NOT IN (${placeholders})`,
                    [passageId, username, ...keepIds]
                );
            }
        }
        schedulerLogSummary(`[focus prune] Pruned focus_submission to top ${n} per user/passage at ${new Date().toISOString()}`);
    } catch (err) {
        console.error('[focus prune] pruneFocusSubmissionsTopN failed:', err);
    }
}

function scheduleDailyPruneAt(hour = 3, minute = 0, n = 10) {
    try {
        const now = new Date();
        const next = new Date();
        ...
        setTimeout(() => {
            pruneFocusSubmissionsTopN(n);
            setInterval(() => pruneFocusSubmissionsTopN(n), 24 * 60 * 60 * 1000);
        }, delay);
        schedulerLogSummary(`[focus prune] Scheduled daily prune at ${hour}:${String(minute).padStart(2,'0')} (first run in ${Math.round(delay/1000)}s)`);
    } catch (err) {
        console.error('[focus prune] scheduleDailyPruneAt failed:', err);
    }
}

// Initial run shortly after startup, then schedule daily at 03:00
setTimeout(() => pruneFocusSubmissionsTopN(10), 10 * 1000);
scheduleDailyPruneAt(3, 0, 10);
...
db.serialize(() => {
    db.run(`
        CREATE TABLE IF NOT EXISTS reading_arena_major_level (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            major_level INTEGER NOT NULL UNIQUE,
            created_at TEXT NOT NULL,
            updated_at TEXT NOT NULL
        )
    `);
    db.run(`
        CREATE TABLE IF NOT EXISTS reading_arena_minor_level (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            major_id INTEGER NOT NULL,
            minor_level INTEGER NOT NULL,
            created_at TEXT NOT NULL,
            updated_at TEXT NOT NULL,
            UNIQUE(major_id, minor_level),
            FOREIGN KEY(major_id) REFERENCES reading_arena_major_level(id)
        )
    `);
    db.run(`
        CREATE TABLE IF NOT EXISTS reading_arena_passage (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            minor_id INTEGER NOT NULL UNIQUE,
            content TEXT NOT NULL,
            word_count INTEGER NOT NULL DEFAULT 0,
            created_at TEXT NOT NULL,
            updated_at TEXT NOT NULL,
            FOREIGN KEY(minor_id) REFERENCES reading_arena_minor_level(id)
        )
    `);
    db.run(`
        CREATE TABLE IF NOT EXISTS reading_arena_record (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            username TEXT NOT NULL,
            ...
            match_outcome TEXT DEFAULT '',
            answer_phase TEXT DEFAULT 'main',
            answer_type TEXT DEFAULT 'read',
            sentences TEXT DEFAULT '[]',
            user_answers TEXT DEFAULT '[]',
            ai_judges TEXT DEFAULT '[]',
            ai_comments TEXT DEFAULT '[]',
            recommended_translations TEXT DEFAULT '[]',
            ai_report TEXT DEFAULT '',
            audio_filenames TEXT DEFAULT '[]',
            progress_trace TEXT DEFAULT '[]',
            submit_time TEXT,
            created_at TEXT NOT NULL,
            FOREIGN KEY(passage_id) REFERENCES reading_arena_passage(id)
        )
    `);
    db.run(`
        CREATE TABLE IF NOT EXISTS reading_arena_daily_attempt (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            username TEXT NOT NULL,
    ...
    `);
    db.run(`
        CREATE TABLE IF NOT EXISTS reading_arena_read_count_override (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            username TEXT NOT NULL,
            major_level INTEGER NOT NULL,
            minor_level INTEGER NOT NULL,
            override_read_count INTEGER NOT NULL DEFAULT 0,
            updated_at TEXT NOT NULL,
            created_at TEXT NOT NULL,
            UNIQUE(username, major_level, minor_level)
        )
    `);
    db.run(`ALTER TABLE reading_arena_passage ADD COLUMN word_count INTEGER NOT NULL DEFAULT 0`, () => {});
    db.run(`ALTER TABLE reading_arena_record ADD COLUMN passage_word_count INTEGER NOT NULL DEFAULT 0`, () => {});
    db.run(`ALTER TABLE reading_arena_record ADD COLUMN error_count INTEGER NOT NULL DEFAULT 0`, () => {});
    db.run(`ALTER TABLE reading_arena_record ADD COLUMN accuracy_rate REAL`, () => {});
    db.run(`ALTER TABLE reading_arena_record ADD COLUMN duration_seconds INTEGER NOT NULL DEFAULT 0`, () => {});
    db.run(`ALTER TABLE reading_arena_record ADD COLUMN audio_count INTEGER NOT NULL DEFAULT 0`, () => {});
    db.run(`ALTER TABLE reading_arena_record ADD COLUMN power_award REAL`, () => {});
    db.run(`ALTER TABLE reading_arena_record ADD COLUMN power_multiplier REAL`, () => {});
    db.run(`ALTER TABLE reading_arena_record ADD COLUMN match_outcome TEXT DEFAULT ''`, () => {});
    db.run(`ALTER TABLE reading_arena_record ADD COLUMN answer_phase TEXT DEFAULT 'main'`, () => {});
    db.run(`ALTER TABLE reading_arena_record ADD COLUMN answer_type TEXT DEFAULT 'read'`, () => {});
    db.run(`ALTER TABLE reading_arena_record ADD COLUMN sentences TEXT DEFAULT '[]'`, () => {});
    db.run(`ALTER TABLE reading_arena_record ADD COLUMN user_answers TEXT DEFAULT '[]'`, () => {});
    db.run(`ALTER TABLE reading_arena_record ADD COLUMN ai_judges TEXT DEFAULT '[]'`, () => {});
    db.run(`ALTER TABLE reading_arena_record ADD COLUMN ai_comments TEXT DEFAULT '[]'`, () => {});
    db.run(`ALTER TABLE reading_arena_record ADD COLUMN recommended_translations TEXT DEFAULT '[]'`, () => {});
    db.run(`ALTER TABLE reading_arena_record ADD COLUMN ai_report TEXT DEFAULT ''`, () => {});
    db.run(`ALTER TABLE reading_arena_record ADD COLUMN audio_filenames TEXT DEFAULT '[]'`, () => {});
    db.run(`ALTER TABLE reading_arena_record ADD COLUMN progress_trace TEXT DEFAULT '[]'`, () => {});
    db.run(`ALTER TABLE reading_arena_record ADD COLUMN submit_time TEXT`, () => {});
    db.run(`ALTER TABLE reading_arena_record ADD COLUMN created_at TEXT`, () => {});
    db.run(`CREATE INDEX IF NOT EXISTS idx_reading_arena_minor_major ON reading_arena_minor_level(major_id, minor_level)`);
    db.run(`CREATE INDEX IF NOT EXISTS idx_reading_arena_passage_minor ON reading_arena_passage(minor_id)`);
    db.run(`CREATE INDEX IF NOT EXISTS idx_reading_arena_record_level ON reading_arena_record(major_level, minor_level, created_at)`);
    db.run(`CREATE INDEX IF NOT EXISTS idx_reading_arena_record_rank ON reading_arena_record(major_level, minor_level, error_count, duration_seconds, audio_count, submit_time)`);
    db.run(`CREATE INDEX IF NOT EXISTS idx_reading_arena_record_user ON reading_arena_record(username, created_at)`);
    db.run(`CREATE INDEX IF NOT EXISTS idx_reading_arena_record_passage ON reading_arena_record(passage_id, created_at)`);
    db.run(`CREATE INDEX IF NOT EXISTS idx_reading_arena_daily_attempt_user_date ON reading_arena_daily_attempt(username, usage_date, created_at)`);
    db.run(`CREATE INDEX IF NOT EXISTS idx_reading_arena_override_user_level ON reading_arena_read_count_override(username, major_level, minor_level)`);
});

function countReadingArenaWords(content) {
    const text = typeof content === 'string' ? content.trim() : '';
    if (!text) return 0;
    return text.split(/\s+/).filter(Boolean).length;
}

const READING_ARENA_UNLOCK_ACCURACY_THRESHOLD = 0.9;
const READING_ARENA_POWER_PER_WORD = 0.001;
const READING_ARENA_DAILY_ATTEMPT_LIMIT = 10;
const READING_ARENA_COOLDOWN_TRIGGER_COUNT = 30;
const READING_ARENA_COOLDOWN_DAYS = 60;
const READING_ARENA_COOLDOWN_MS = READING_ARENA_COOLDOWN_DAYS * 24 * 60 * 60 * 1000;

function getReadingArenaUsageDateKey(date = new Date()) {
    const safeDate = date instanceof Date ? date : new Date(date);
    const beijingTime = new Date(safeDate.getTime() + 8 * 60 * 60 * 1000);
    return beijingTime.toISOString().slice(0, 10);
}

async function getReadingArenaDailyAttemptSummary(username, usageDate = getReadingArenaUsageDateKey()) {
    const normalizedUsername = typeof username === 'string' ? username.trim() : '';
    if (!normalizedUsername) {
        return {
            usageDate,
            limit: READING_ARENA_DAILY_ATTEMPT_LIMIT,
            used: 0,
            remaining: 0,
            canStartToday: false
        };
    ...
    const row = await dbGet(
        `SELECT COUNT(1) AS used_count
         FROM reading_arena_daily_attempt
         WHERE username = ? AND usage_date = ?`,
        [normalizedUsername, usageDate]
    );

    const used = Math.max(0, Number(row && row.used_count ? row.used_count : 0));
    const remaining = Math.max(0, READING_ARENA_DAILY_ATTEMPT_LIMIT - used);
    return {
        usageDate,
        limit: READING_ARENA_DAILY_ATTEMPT_LIMIT,
        used,
        remaining,
        canStartToday: remaining > 0
    };
}

async function consumeReadingArenaDailyAttempt(username, majorLevel, minorLevel) {
    const normalizedUsername = typeof username === 'string' ? username.trim() : '';
    if (!normalizedUsername) {
        return {
            success: false,
            error: '缺少有效的 username',
            summary: await getReadingArenaDailyAttemptSummary('')
        };
    }

    const usageDate = getReadingArenaUsageDateKey();
    const now = nowISO();
    const result = await dbRun(
        `INSERT INTO reading_arena_daily_attempt (
            username, usage_date, major_level, minor_level, consumed_at, created_at
         )
         SELECT ?, ?, ?, ?, ?, ?
         WHERE (
            SELECT COUNT(1)
        ...
        [
            normalizedUsername,
            usageDate,
            normalizePositiveInteger(majorLevel),
            normalizePositiveInteger(minorLevel),
            now,
            now,
            normalizedUsername,
            usageDate,
            READING_ARENA_DAILY_ATTEMPT_LIMIT
        ]
    );

    const summary = await getReadingArenaDailyAttemptSummary(normalizedUsername, usageDate);
    if (!result || !Number(result.changes)) {
        return {
            success: false,
            error: '今日朗读角斗场次数已用完',
            summary
...
function calculateReadingArenaAccuracyRate(wordCount, errorCount) {
    const totalWords = Number(wordCount);
    if (!Number.isFinite(totalWords) || totalWords <= 0) return 0;

    const safeErrorCount = Number.isFinite(Number(errorCount)) ? Math.max(0, Number(errorCount)) : 0;
    const accuracyRate = (totalWords - safeErrorCount) / totalWords;
    return Math.max(0, Math.min(1, accuracyRate));
}

async function getReadingArenaBestAccuracyMap(username) {
    const normalizedUsername = typeof username === 'string' ? username.trim() : '';
    if (!normalizedUsername) return new Map();

    const rows = await dbAll(
        `SELECT major_level, minor_level, passage_word_count, error_count
         FROM reading_arena_record
         WHERE username = ?`,
        [normalizedUsername]
    );

    const bestAccuracyMap = new Map();
    for (const row of rows || []) {
        const levelKey = `${Number(row.major_level)}-${Number(row.minor_level)}`;
        const accuracyRate = calculateReadingArenaAccuracyRate(row.passage_word_count, row.error_count);
        const previousAccuracyRate = Number(bestAccuracyMap.get(levelKey) || 0);
        if (accuracyRate > previousAccuracyRate) {
            bestAccuracyMap.set(levelKey, accuracyRate);
        }
    }

    return bestAccuracyMap;
}

async function getReadingArenaBestRankMap(username) {
    const normalizedUsername = typeof username === 'string' ? username.trim() : '';
    if (!normalizedUsername) return new Map();

    const rows = await dbAll(
        `SELECT username, major_level, minor_level
         FROM reading_arena_record
         ORDER BY major_level ASC,
                  minor_level ASC,
                  error_count ASC,
                  duration_seconds ASC,
                  audio_count ASC,
                  COALESCE(submit_time, created_at) ASC,
                  id ASC`
    );

    const bestRankMap = new Map();
    let currentLevelKey = '';
    let currentRank = 0;
    ...
    for (const row of rows || []) {
        ...
        if (row.username !== normalizedUsername) continue;

        const previousBestRank = bestRankMap.get(levelKey);
        if (!Number.isInteger(previousBestRank) || currentRank < previousBestRank) {
            bestRankMap.set(levelKey, currentRank);
        }
    }
...
function buildReadingArenaCooldownInfo(readCount, cooldownBaseAt) {
    const safeReadCount = Math.max(0, Number(readCount || 0));
    const cooldownBaseAtText = typeof cooldownBaseAt === 'string' ? cooldownBaseAt : '';
    const cooldownBaseAtMs = Date.parse(cooldownBaseAtText);
    const cooldownReached = safeReadCount >= READING_ARENA_COOLDOWN_TRIGGER_COUNT;
    const cooldownEndsAtMs = cooldownReached && Number.isFinite(cooldownBaseAtMs)
        ? cooldownBaseAtMs + READING_ARENA_COOLDOWN_MS
        : 0;
    const cooldownActive = Boolean(cooldownReached && cooldownEndsAtMs > Date.now());
    const cooldownRemainingDays = cooldownActive
        ? Math.max(1, Math.ceil((cooldownEndsAtMs - Date.now()) / (24 * 60 * 60 * 1000)))
        : 0;
    ...
    return {
        ...
        cooldownStartedAt: cooldownReached ? cooldownBaseAtText : '',
        cooldownEndsAt: cooldownReached && cooldownEndsAtMs > 0 ? new Date(cooldownEndsAtMs).toISOString() : '',
        cooldownReason: cooldownActive ? `该等级已进入冷却状态，剩${cooldownRemainingDays}天可再次朗读。` : ''
    };
}

async function getReadingArenaActualReadCountMap(username) {
    const normalizedUsername = typeof username === 'string' ? username.trim() : '';
    if (!normalizedUsername) return new Map();
    ...
    const rows = await dbAll(
        `SELECT major_level,
                minor_level,
                COALESCE(submit_time, created_at) AS completed_at
         FROM reading_arena_record
         WHERE username = ?
         ORDER BY major_level ASC,
                  minor_level ASC,
    ...
    for (const row of rows || []) {
        const levelKey = `${Number(row.major_level)}-${Number(row.minor_level)}`;
        const currentSummary = summaryMap.get(levelKey) || {
            actualReadCount: 0,
            actualLatestCompletedAt: '',
            actualCooldownBaseAt: ''
        };
        currentSummary.actualReadCount += 1;
        currentSummary.actualLatestCompletedAt = row.completed_at || '';
        if (currentSummary.actualReadCount % READING_ARENA_COOLDOWN_TRIGGER_COUNT === 0) {
            currentSummary.actualCooldownBaseAt = row.completed_at || '';
        }
        summaryMap.set(levelKey, currentSummary);
    }

    return summaryMap;
}

async function getReadingArenaReadCountOverrideMap(username) {
    const normalizedUsername = typeof username === 'string' ? username.trim() : '';
    if (!normalizedUsername) return new Map();

    const rows = await dbAll(
        `SELECT username, major_level, minor_level, override_read_count, updated_at, created_at
         FROM reading_arena_read_count_override
         WHERE username = ?`,
        [normalizedUsername]
    );
    ...
    for (const row of rows || []) {
        const levelKey = `${Number(row.major_level)}-${Number(row.minor_level)}`;
        overrideMap.set(levelKey, {
            overrideReadCount: Math.max(0, Number(row.override_read_count || 0)),
            overrideUpdatedAt: row.updated_at || row.created_at || ''
        });
    }
...
function buildReadingArenaEffectiveReadCountSummary(actualSummary, overrideSummary) {
    const normalizedActual = actualSummary || {};
    const normalizedOverride = overrideSummary || null;
    const actualReadCount = Math.max(0, Number(normalizedActual.actualReadCount || 0));
    const actualLatestCompletedAt = normalizedActual.actualLatestCompletedAt || '';
    const actualCooldownBaseAt = normalizedActual.actualCooldownBaseAt || '';
    const hasOverride = Boolean(normalizedOverride);
    const overrideReadCount = hasOverride ? Math.max(0, Number(normalizedOverride.overrideReadCount || 0)) : null;
    const overrideUpdatedAt = hasOverride ? (normalizedOverride.overrideUpdatedAt || '') : '';
    const effectiveReadCount = hasOverride ? overrideReadCount : actualReadCount;
    const effectiveLatestCompletedAt = hasOverride ? overrideUpdatedAt : actualLatestCompletedAt;
    const effectiveCooldownBaseAt = hasOverride
        ? (overrideReadCount >= READING_ARENA_COOLDOWN_TRIGGER_COUNT ? overrideUpdatedAt : '')
        : actualCooldownBaseAt;
    const cooldown = buildReadingArenaCooldownInfo(effectiveReadCount, effectiveCooldownBaseAt);
    ...
    return {
        ...
        overrideUpdatedAt,
        effectiveReadCount,
        effectiveLatestCompletedAt,
        effectiveCooldownBaseAt,
        cooldown
    };
}

async function getReadingArenaEffectiveReadCountMap(username) {
    const actualMap = await getReadingArenaActualReadCountMap(username);
    const overrideMap = await getReadingArenaReadCountOverrideMap(username);
    const keys = new Set([...actualMap.keys(), ...overrideMap.keys()]);
    const effectiveMap = new Map();

    for (const key of keys) {
        effectiveMap.set(
            key,
            buildReadingArenaEffectiveReadCountSummary(actualMap.get(key), overrideMap.get(key))
        );
    }
...
async function getReadingArenaEffectiveReadCountSummary(username, majorLevel, minorLevel) {
    const normalizedUsername = typeof username === 'string' ? username.trim() : '';
    if (!normalizedUsername) {
        return buildReadingArenaEffectiveReadCountSummary(null, null);
    }

    const levelKey = `${Number(majorLevel)}-${Number(minorLevel)}`;
    const actualMap = await getReadingArenaActualReadCountMap(normalizedUsername);
    const overrideMap = await getReadingArenaReadCountOverrideMap(normalizedUsername);
    return buildReadingArenaEffectiveReadCountSummary(actualMap.get(levelKey), overrideMap.get(levelKey));
}

async function getReadingArenaCooldownMap(username) {
    const effectiveMap = await getReadingArenaEffectiveReadCountMap(username);
    const cooldownMap = new Map();
    for (const [levelKey, summary] of effectiveMap.entries()) {
        cooldownMap.set(levelKey, summary.cooldown);
    }

    return cooldownMap;
}

async function getReadingArenaCooldownStatus(username, majorLevel, minorLevel) {
    const normalizedUsername = typeof username === 'string' ? username.trim() : '';
    if (!normalizedUsername) {
        return buildReadingArenaCooldownInfo(0, '');
    }

    const summary = await getReadingArenaEffectiveReadCountSummary(normalizedUsername, majorLevel, minorLevel);
    return summary.cooldown;
}

async function ensureReadingArenaLevelNotCooling(username, majorLevel, minorLevel) {
    const cooldown = await getReadingArenaCooldownStatus(username, majorLevel, minorLevel);
    if (cooldown.cooldownActive) {
        return {
            success: false,
            error: cooldown.cooldownReason,
            cooldown
        };
    }
...
async function getReadingArenaPowerValue(username) {
    const normalizedUsername = typeof username === 'string' ? username.trim() : '';
    if (!normalizedUsername) return 0;

    const row = await dbGet(
        `SELECT COALESCE(SUM(COALESCE(power_award, COALESCE(passage_word_count, 0) * ?)), 0) AS total_power
         FROM reading_arena_record
         WHERE username = ?`,
        [READING_ARENA_POWER_PER_WORD, normalizedUsername]
    );

    const totalPower = Number(row && row.total_power ? row.total_power : 0);
    if (!Number.isFinite(totalPower) || totalPower <= 0) return 0;
    return Number(totalPower.toFixed(3));
}

async function syncReadingArenaWordCounts() {
    try {
        const passages = await dbAll(`SELECT id, content FROM reading_arena_passage`);
        for (const passage of passages || []) {
            const wordCount = countReadingArenaWords(passage.content);
            await dbRun(`UPDATE reading_arena_passage SET word_count = ? WHERE id = ?`, [wordCount, passage.id]);
        }
    } catch (err) {
        console.error('同步朗读角斗场文章单词数失败:', err);
    }
}

syncReadingArenaWordCounts();
...
function findNextAvailablePositiveInteger(values) {
    const used = new Set((values || []).map(v => Number(v)).filter(v => Number.isInteger(v) && v > 0));
    let next = 1;
    while (used.has(next)) next++;
    return next;
}

async function getReadingArenaMajorByLevel(majorLevel) {
    return dbGet(
        `SELECT id, major_level, created_at, updated_at
         FROM reading_arena_major_level
         WHERE major_level = ?
         LIMIT 1`,
        [majorLevel]
    );
...
async function getReadingArenaMinorByLevels(majorLevel, minorLevel) {
    return dbGet(
        `SELECT mil.id, mil.major_id, mil.minor_level, mal.major_level, mil.created_at, mil.updated_at
         FROM reading_arena_minor_level mil
         INNER JOIN reading_arena_major_level mal ON mal.id = mil.major_id
         WHERE mal.major_level = ? AND mil.minor_level = ?
         LIMIT 1`,
        [majorLevel, minorLevel]
    );
}

async function getReadingArenaPassageByMinorId(minorId) {
    return dbGet(
        `SELECT id, minor_id, content, word_count, created_at, updated_at
         FROM reading_arena_passage
         WHERE minor_id = ?
         LIMIT 1`,
        [minorId]
    );
...
function normalizeReadingArenaStringArray(value) {
...
function normalizeReadingArenaProgressTrace(value) {
    if (!Array.isArray(value)) return [];

    return value.map((item, index) => {
        if (Number.isFinite(item) && Number(item) >= 0) {
            const elapsedMs = Math.round(Number(item));
            return {
                order: index + 1,
                elapsedMs,
        ...
        const wordIndex = normalizeNonNegativeInteger(item.wordIndex ?? item.word_index ?? item.index);
        const rawWord = typeof item.word === 'string' ? item.word.trim() : '';
        const elapsedMsInput = Number(item.elapsedMs ?? item.elapsed_ms ?? item.timeMs ?? item.time_ms);
        const elapsedSecondsInput = Number(item.elapsedSeconds ?? item.elapsed_seconds ?? item.seconds);

        let elapsedMs = null;
        if (Number.isFinite(elapsedMsInput) && elapsedMsInput >= 0) {
            elapsedMs = Math.round(elapsedMsInput);
        } else if (Number.isFinite(elapsedSecondsInput) && elapsedSecondsInput >= 0) {
            elapsedMs = Math.round(elapsedSecondsInput * 1000);
        }
        ...
        if (wordIndex != null) traceItem.wordIndex = wordIndex;
        if (rawWord) traceItem.word = rawWord;

        return traceItem;
    }).filter(Boolean);
}

function extractReadingArenaErrorCount(body, aiComments, aiReport) {
    const directValue = normalizeNonNegativeInteger(body.errorCount ?? body.error_count ?? body.totalErrors ?? body.total_errors);
    if (directValue != null) return directValue;

    const commentText = Array.isArray(aiComments) ? aiComments.join(' ') : '';
    const reportText = typeof aiReport === 'string' ? aiReport : '';
    const mergedText = `${commentText} ${reportText}`;
    const match = mergedText.match(/错误(?:次数)?[：:\s]*([0-9]+)/i);
    if (match) {
        const parsed = normalizeNonNegativeInteger(match[1]);
...
function extractReadingArenaDurationSeconds(body, progressTrace) {
    const directValue = normalizeNonNegativeInteger(body.durationSeconds ?? body.duration_seconds);
    if (directValue != null) return directValue;

    const elapsedMsList = Array.isArray(progressTrace)
        ? progressTrace
            .map(item => Number(item && item.elapsedMs))
            .filter(value => Number.isFinite(value) && value >= 0)
        : [];
    if (elapsedMsList.length > 0) {
        return Math.max(0, Math.round(Math.max(...elapsedMsList) / 1000));
    }

    return 0;
}

function extractReadingArenaAudioCount(body, audioFilenames) {
    const directValue = normalizeNonNegativeInteger(body.audioCount ?? body.audio_count);
    if (directValue != null) return directValue;
    return Array.isArray(audioFilenames) ? audioFilenames.length : 0;
}

function extractReadingArenaAccuracyRate(body, passageWordCount, errorCount) {
    const directValue = Number(body.accuracyRate ?? body.accuracy_rate);
    if (Number.isFinite(directValue) && directValue >= 0) {
        return Math.max(0, Math.min(1, directValue));
    }
    return calculateReadingArenaAccuracyRate(passageWordCount, errorCount);
}

function extractReadingArenaPowerAward(body, passageWordCount, accuracyRate) {
    const directValue = Number(body.powerAward ?? body.power_award);
    if (Number.isFinite(directValue) && directValue >= 0) {
        return Number(directValue.toFixed(6));
    }

    const multiplier = accuracyRate >= READING_ARENA_UNLOCK_ACCURACY_THRESHOLD ? 1 : 0.8;
    return Number((Number(passageWordCount || 0) * READING_ARENA_POWER_PER_WORD * multiplier).toFixed(6));
}

function extractReadingArenaPowerMultiplier(body, accuracyRate) {
    const directValue = Number(body.powerMultiplier ?? body.power_multiplier);
    if (Number.isFinite(directValue) && directValue >= 0) {
        return Number(directValue.toFixed(4));
    }
    return accuracyRate >= READING_ARENA_UNLOCK_ACCURACY_THRESHOLD ? 1 : 0.8;
}

function extractReadingArenaMatchOutcome(body) {
    const value = typeof (body.matchOutcome ?? body.match_outcome) === 'string'
        ? String(body.matchOutcome ?? body.match_outcome).trim()
        : '';
    return value;
}

function serializeReadingArenaRecordRow(row) {
    return {
        id: Number(row.id),
        username: row.username,
        majorLevel: Number(row.major_level),
        minorLevel: Number(row.minor_level),
        levelLabel: `${row.major_level}-${row.minor_level}`,
        passageId: row.passage_id == null ? null : Number(row.passage_id),
        passageWordCount: Number(row.passage_word_count || 0),
        errorCount: Number(row.error_count || 0),
        accuracyRate: Number(row.accuracy_rate || 0),
        durationSeconds: Number(row.duration_seconds || 0),
        audioCount: Number(row.audio_count || 0),
        powerAward: Number(row.power_award || 0),
        powerMultiplier: Number(row.power_multiplier || 0),
        matchOutcome: row.match_outcome || '',
        answerPhase: row.answer_phase || 'main',
        answerType: row.answer_type || 'read',
        sentences: safeParseJson(row.sentences),
        userAnswers: safeParseJson(row.user_answers),
        aiJudges: safeParseJson(row.ai_judges),
        aiComments: safeParseJson(row.ai_comments),
        recommendedTranslations: safeParseJson(row.recommended_translations),
        aiReport: row.ai_report || '',
        audioFilenames: safeParseJson(row.audio_filenames),
        progressTrace: safeParseJson(row.progress_trace),
        submitTime: row.submit_time || '',
        createdAt: row.created_at || ''
    };
}

async function getReadingArenaRankingRows(majorLevel, minorLevel, limit = null) {
    const params = [majorLevel, minorLevel];
    let sql = `SELECT *
               FROM reading_arena_record
               WHERE major_level = ? AND minor_level = ?
...
function getReadingArenaRecordTimestampMs(record) {
    const rawValue = record ? (record.submitTime || record.submit_time || record.createdAt || record.created_at || '') : '';
    const parsed = Date.parse(rawValue);
    return Number.isNaN(parsed) ? 0 : parsed;
}

function buildReadingArenaRankedRecord(row, index) {
    return {
        rank: index + 1,
        ...serializeReadingArenaRecordRow(row),
        completedTime: row.submit_time || row.created_at || ''
    };
}

function chooseReadingArenaMatchOpponent(rankedRecords, username) {
    if (!Array.isArray(rankedRecords) || !rankedRecords.length) {
        return {
            strategy: 'no-records',
    ...
    const userRecords = rankedRecords.filter(record => record.username === username);
    if (!userRecords.length) {
        return {
            strategy: 'lowest-ranked',
            referenceRecord: null,
            opponent: rankedRecords[rankedRecords.length - 1],
            candidateCount: 1
        };
    }

    const latestUserRecord = userRecords.slice().sort((left, right) => {
        const timeDiff = getReadingArenaRecordTimestampMs(right) - getReadingArenaRecordTimestampMs(left);
        if (timeDiff !== 0) return timeDiff;
        return Number(right.id || 0) - Number(left.id || 0);
    })[0];

    const latestIndex = rankedRecords.findIndex(record => record.id === latestUserRecord.id);
    const higherRecords = [];
    const lowerRecords = [];

    for (let index = latestIndex - 1; index >= 0 && higherRecords.length < 5; index -= 1) {
        if (rankedRecords[index].username !== username) {
            higherRecords.unshift(rankedRecords[index]);
        }
    }

    for (let index = latestIndex + 1; index < rankedRecords.length && lowerRecords.length < 5; index += 1) {
        if (rankedRecords[index].username !== username) {
            lowerRecords.push(rankedRecords[index]);
        }
    }

    const candidates = [...higherRecords, ...lowerRecords];
    const fallbackCandidates = rankedRecords.filter(record => record.username !== username);
    const pool = candidates.length ? candidates : fallbackCandidates;

    if (!pool.length) {
        return {
            strategy: 'around-latest-rank',
            referenceRecord: latestUserRecord,
            opponent: null,
            candidateCount: 0
        };
    }
...
async function buildReadingArenaLevelTree(username = '') {
    const rows = await dbAll(
        `SELECT mal.major_level, mil.minor_level,
                CASE WHEN rap.id IS NULL THEN 0 ELSE 1 END AS has_passage,
                COALESCE(rap.word_count, 0) AS word_count
         FROM reading_arena_major_level mal
         LEFT JOIN reading_arena_minor_level mil ON mil.major_id = mal.id
         LEFT JOIN reading_arena_passage rap ON rap.minor_id = mil.id
         ORDER BY mal.major_level ASC, mil.minor_level ASC`
    );

    const majorMap = new Map();
    const bestAccuracyMap = await getReadingArenaBestAccuracyMap(username);
    const bestRankMap = await getReadingArenaBestRankMap(username);
    const cooldownMap = await getReadingArenaCooldownMap(username);
    const readCountSummaryMap = await getReadingArenaEffectiveReadCountMap(username);
    for (const row of rows || []) {
        const majorLevel = Number(row.major_level);
        if (!majorMap.has(majorLevel)) {
            majorMap.set(majorLevel, {
                majorLevel,
                minors: []
            });
        }
        if (row.minor_level != null) {
            const minorLevel = Number(row.minor_level);
            const levelLabel = `${majorLevel}-${minorLevel}`;
            const bestAccuracyRate = Number(bestAccuracyMap.get(levelLabel) || 0);
            const cooldownInfo = cooldownMap.get(levelLabel) || buildReadingArenaCooldownInfo(0, '');
            const readCountSummary = readCountSummaryMap.get(levelLabel) || buildReadingArenaEffectiveReadCountSummary(null, null);
            majorMap.get(majorLevel).minors.push({
                minorLevel,
                hasPassage: Number(row.has_passage) === 1,
                wordCount: Number(row.word_count || 0),
                label: levelLabel,
                bestRank: bestRankMap.has(levelLabel) ? Number(bestRankMap.get(levelLabel)) : null,
                readCount: Number(readCountSummary.effectiveReadCount || 0),
                actualReadCount: Number(readCountSummary.actualReadCount || 0),
                hasReadCountOverride: Boolean(readCountSummary.hasOverride),
                overrideReadCount: readCountSummary.overrideReadCount == null ? null : Number(readCountSummary.overrideReadCount),
                overrideUpdatedAt: readCountSummary.overrideUpdatedAt || '',
                cooldownActive: Boolean(cooldownInfo.cooldownActive),
                cooldownRemainingDays: Number(cooldownInfo.cooldownRemainingDays || 0),
                cooldownStartedAt: cooldownInfo.cooldownStartedAt || '',
                cooldownEndsAt: cooldownInfo.cooldownEndsAt || '',
                cooldownReason: cooldownInfo.cooldownReason || '',
                bestAccuracyRate: Number(bestAccuracyRate.toFixed(4)),
                unlockQualified: bestAccuracyRate >= READING_ARENA_UNLOCK_ACCURACY_THRESHOLD,
                unlocked: false,
                lockedReason: ''
            });
        }
    ...
    const levels = Array.from(majorMap.values())
        .sort((a, b) => a.majorLevel - b.majorLevel)
        .map(level => ({
            ...level,
            minors: (level.minors || []).sort((a, b) => a.minorLevel - b.minorLevel)
        }));

    const orderedMinors = [];
    levels.forEach(level => {
        (level.minors || []).forEach(minor => {
            orderedMinors.push(minor);
        });
    });

    orderedMinors.forEach((minor, index) => {
        if (index === 0) {
            minor.unlocked = true;
            minor.lockedReason = '';
            return;
        ...
        const previousMinor = orderedMinors[index - 1];
        minor.unlocked = Boolean(previousMinor && previousMinor.unlockQualified);
        minor.lockedReason = minor.unlocked
            ? ''
            : `需先完成等级 ${previousMinor.label}，并在任意一次朗读中达到90%正确率。`;
    });

    return levels;
}

function parseReadingArenaLevelLabel(levelLabel) {
    const match = typeof levelLabel === 'string' ? levelLabel.match(/^(\d+)-(\d+)$/) : null;
    if (!match) {
        return { majorLevel: 0, minorLevel: 0 };
    }
    return {
        majorLevel: Number(match[1]),
        minorLevel: Number(match[2])
    };
}

async function getReadingArenaHighestUnlockedLevelLabel(username) {
    const normalizedUsername = typeof username === 'string' ? username.trim() : '';
    if (!normalizedUsername) return '--';

    const levels = await buildReadingArenaLevelTree(normalizedUsername);
    const unlockedMinors = [];
    (levels || []).forEach(level => {
        (level.minors || []).forEach(minor => {
            if (minor && minor.unlocked) {
                unlockedMinors.push(minor);
            }
        });
    });

    if (!unlockedMinors.length) return '--';
    const highestMinor = unlockedMinors[unlockedMinors.length - 1];
    return highestMinor && highestMinor.label ? String(highestMinor.label) : '--';
}

async function getReadingArenaPowerRanking(limit = 30) {
    const safeLimit = Math.min(30, normalizePositiveInteger(limit) || 30);
    const rows = await dbAll(
        `SELECT username,
                COALESCE(SUM(COALESCE(power_award, COALESCE(passage_word_count, 0) * ?)), 0) AS total_power,
                MAX(COALESCE(submit_time, created_at)) AS latest_completed_at
         FROM reading_arena_record
         GROUP BY username
         HAVING COUNT(1) > 0`,
        [READING_ARENA_POWER_PER_WORD]
    );

    const enrichedRows = await Promise.all((rows || []).map(async row => {
        const highestUnlockedLevel = await getReadingArenaHighestUnlockedLevelLabel(row.username);
        const parsedHighestLevel = parseReadingArenaLevelLabel(highestUnlockedLevel);
        const powerValue = Number(Number(row.total_power || 0).toFixed(3));
        return {
            username: row.username,
            powerValue: Number.isFinite(powerValue) ? powerValue : 0,
            highestUnlockedLevel,
            highestMajorLevel: parsedHighestLevel.majorLevel,
            highestMinorLevel: parsedHighestLevel.minorLevel,
            latestCompletedAt: row.latest_completed_at || ''
        };
    }));

    return enrichedRows
        .filter(row => row.username && row.powerValue > 0)
        .sort((left, right) => {
            if (right.powerValue !== left.powerValue) return right.powerValue - left.powerValue;
            if (right.highestMajorLevel !== left.highestMajorLevel) return right.highestMajorLevel - left.highestMajorLevel;
            if (right.highestMinorLevel !== left.highestMinorLevel) return right.highestMinorLevel - left.highestMinorLevel;
            return getReadingArenaRecordTimestampMs({ submitTime: right.latestCompletedAt }) - getReadingArenaRecordTimestampMs({ submitTime: left.latestCompletedAt });
        })
        .slice(0, safeLimit)
        .map((row, index) => ({
            rank: index + 1,
...
app.get('/api/reading-arena/levels', async (req, res) => {
    try {
        const username = typeof req.query.username === 'string' ? req.query.username.trim() : '';
        const levels = await buildReadingArenaLevelTree(username);
        const readingPower = await getReadingArenaPowerValue(username);
        res.json({ success: true, levels, readingPower });
    } catch (err) {
        console.error('获取朗读角斗场等级列表失败:', err);
        res.status(500).json({ success: false, error: err.message || '获取等级列表失败' });
    }
});

app.get('/api/reading-arena/read-count', async (req, res) => {
    try {
        const username = typeof req.query.username === 'string' ? req.query.username.trim() : '';
        const majorLevel = normalizePositiveInteger(req.query.majorLevel);
        const minorLevel = normalizePositiveInteger(req.query.minorLevel);

        if (!username) {
            return res.status(400).json({ success: false, error: '缺少有效的 username' });
        }
        if (!majorLevel || !minorLevel) {
            return res.status(400).json({ success: false, error: '缺少有效的 majorLevel 或 minorLevel' });
        }

        const summary = await getReadingArenaEffectiveReadCountSummary(username, majorLevel, minorLevel);
        res.json({
            success: true,
            summary: {
                username,
                majorLevel,
                minorLevel,
                levelLabel: `${majorLevel}-${minorLevel}`,
                actualReadCount: Number(summary.actualReadCount || 0),
                actualLatestCompletedAt: summary.actualLatestCompletedAt || '',
                effectiveReadCount: Number(summary.effectiveReadCount || 0),
                effectiveLatestCompletedAt: summary.effectiveLatestCompletedAt || '',
                hasOverride: Boolean(summary.hasOverride),
                overrideReadCount: summary.overrideReadCount == null ? null : Number(summary.overrideReadCount),
                overrideUpdatedAt: summary.overrideUpdatedAt || '',
                cooldown: summary.cooldown
            }
        });
    } catch (err) {
        console.error('获取朗读角斗场朗读次数失败:', err);
        res.status(500).json({ success: false, error: err.message || '获取朗读次数失败' });
    }
});

app.post('/api/reading-arena/read-count/override', async (req, res) => {
    try {
        const body = req.body || {};
        const username = typeof body.username === 'string' ? body.username.trim() : '';
        const majorLevel = normalizePositiveInteger(body.majorLevel);
        const minorLevel = normalizePositiveInteger(body.minorLevel);
        const rawOverrideReadCount = body.overrideReadCount;
        const shouldClearOverride = rawOverrideReadCount === '' || rawOverrideReadCount == null;
        ...
        const minorRow = await getReadingArenaMinorByLevels(majorLevel, minorLevel);
        if (!minorRow) {
            return res.status(404).json({ success: false, error: '该等级不存在' });
        }

        if (shouldClearOverride) {
            await dbRun(
                `DELETE FROM reading_arena_read_count_override
                 WHERE username = ? AND major_level = ? AND minor_level = ?`,
                [username, majorLevel, minorLevel]
            );
        } else {
            const overrideReadCount = normalizeNonNegativeInteger(rawOverrideReadCount);
            if (overrideReadCount == null) {
                return res.status(400).json({ success: false, error: 'overrideReadCount 需为大于等于0的整数，或留空表示清除覆盖' });
            }

            const now = nowISO();
            await dbRun(
                `INSERT INTO reading_arena_read_count_override (
                    username, major_level, minor_level, override_read_count, updated_at, created_at
                 ) VALUES (?, ?, ?, ?, ?, ?)
                 ON CONFLICT(username, major_level, minor_level) DO UPDATE SET
                    override_read_count = excluded.override_read_count,
                    updated_at = excluded.updated_at`,
                [username, majorLevel, minorLevel, overrideReadCount, now, now]
            );
        }
        ...
        console.error('设置朗读角斗场朗读次数覆盖失败:', err);
        res.status(500).json({ success: false, error: err.message || '设置朗读次数覆盖失败' });
    }
});

app.post('/api/reading-arena/major-level/add', async (req, res) => {
    try {
        const requestedMajorLevel = req.body ? normalizePositiveInteger(req.body.majorLevel) : null;
        const existingRows = await dbAll(`SELECT major_level FROM reading_arena_major_level ORDER BY major_level ASC`);
        const targetMajorLevel = requestedMajorLevel || findNextAvailablePositiveInteger(existingRows.map(row => row.major_level));
        const existingMajor = await getReadingArenaMajorByLevel(targetMajorLevel);
        if (existingMajor) {
            return res.status(409).json({ success: false, error: '该大等级已存在' });
        }

        const now = nowISO();
        const result = await dbRun(
            `INSERT INTO reading_arena_major_level (major_level, created_at, updated_at)
             VALUES (?, ?, ?)`,
            [targetMajorLevel, now, now]
        );

        res.json({
            success: true,
            majorLevel: targetMajorLevel,
            id: result.lastID
        });
    } catch (err) {
        console.error('新增朗读角斗场大等级失败:', err);
        res.status(500).json({ success: false, error: err.message || '新增大等级失败' });
    }
});

app.post('/api/reading-arena/minor-level/add', async (req, res) => {
    try {
        const majorLevel = req.body ? normalizePositiveInteger(req.body.majorLevel) : null;
        const requestedMinorLevel = req.body ? normalizePositiveInteger(req.body.minorLevel) : null;
        if (!majorLevel) {
            return res.status(400).json({ success: false, error: '缺少有效的 majorLevel' });
        }

        const majorRow = await getReadingArenaMajorByLevel(majorLevel);
        if (!majorRow) {
            return res.status(404).json({ success: false, error: '对应的大等级不存在，请先新增大等级' });
        }

        const existingRows = await dbAll(
            `SELECT minor_level FROM reading_arena_minor_level WHERE major_id = ? ORDER BY minor_level ASC`,
            [majorRow.id]
        );
        const targetMinorLevel = requestedMinorLevel || findNextAvailablePositiveInteger(existingRows.map(row => row.minor_level));
        const existingMinor = await dbGet(
            `SELECT id FROM reading_arena_minor_level WHERE major_id = ? AND minor_level = ? LIMIT 1`,
            [majorRow.id, targetMinorLevel]
        );
        if (existingMinor) {
            return res.status(409).json({ success: false, error: '该小等级已存在' });
        }

        const now = nowISO();
        const result = await dbRun(
            `INSERT INTO reading_arena_minor_level (major_id, minor_level, created_at, updated_at)
             VALUES (?, ?, ?, ?)`,
            [majorRow.id, targetMinorLevel, now, now]
        );

        res.json({
            success: true,
            majorLevel,
            minorLevel: targetMinorLevel,
            id: result.lastID
        });
    } catch (err) {
        console.error('新增朗读角斗场小等级失败:', err);
        res.status(500).json({ success: false, error: err.message || '新增小等级失败' });
    }
});

app.get('/api/reading-arena/passage', async (req, res) => {
    try {
        const majorLevel = normalizePositiveInteger(req.query.majorLevel);
        const minorLevel = normalizePositiveInteger(req.query.minorLevel);
        if (!majorLevel || !minorLevel) {
        ...
        const minorRow = await getReadingArenaMinorByLevels(majorLevel, minorLevel);
        if (!minorRow) {
            return res.status(404).json({ success: false, error: '该等级不存在，请先新增对应等级' });
        }

        const passage = await dbGet(
        ...
        res.json({
            ...
            majorLevel,
            minorLevel,
            exists: Boolean(passage),
            passage: passage ? {
                id: passage.id,
                content: passage.content,
                wordCount: Number(passage.word_count || 0),
                created_at: passage.created_at,
                updated_at: passage.updated_at
            } : null
        });
    } catch (err) {
        console.error('获取朗读角斗场文章失败:', err);
        res.status(500).json({ success: false, error: err.message || '获取文章失败' });
    }
});

app.post('/api/reading-arena/passage/save', async (req, res) => {
    try {
        const majorLevel = req.body ? normalizePositiveInteger(req.body.majorLevel) : null;
        const minorLevel = req.body ? normalizePositiveInteger(req.body.minorLevel) : null;
        const rawContent = req.body && typeof req.body.content === 'string' ? req.body.content : '';
        ...
        if (!normalizedContent) {
            return res.status(400).json({ success: false, error: '文章内容不能为空' });
        }
        ...
        const existingPassage = await dbGet(
            `SELECT id FROM reading_arena_passage WHERE minor_id = ? LIMIT 1`,
            [minorRow.id]
        );
        const now = nowISO();
        const wordCount = countReadingArenaWords(content);

        if (existingPassage) {
            await dbRun(
                `UPDATE reading_arena_passage
                 SET content = ?, word_count = ?, updated_at = ?
                 WHERE id = ?`,
                [content, wordCount, now, existingPassage.id]
            );
            return res.json({ success: true, mode: 'update', id: existingPassage.id, wordCount });
        }

        const result = await dbRun(
            `INSERT INTO reading_arena_passage (minor_id, content, word_count, created_at, updated_at)
             VALUES (?, ?, ?, ?, ?)`,
            [minorRow.id, content, wordCount, now, now]
        );

        res.json({ success: true, mode: 'create', id: result.lastID, wordCount });
    } catch (err) {
        console.error('保存朗读角斗场文章失败:', err);
        res.status(500).json({ success: false, error: err.message || '保存文章失败' });
    }
});

app.post('/api/reading-arena/record/save', async (req, res) => {
    try {
        const body = req.body || {};
        const username = typeof body.username === 'string' ? body.username.trim() : '';
        const majorLevel = normalizePositiveInteger(body.majorLevel);
        ...
        const passageRow = await getReadingArenaPassageByMinorId(minorRow.id);
        if (!passageRow) {
            return res.status(404).json({ success: false, error: '该等级下暂无文章，无法保存朗读记录' });
        }

        const cooldownCheck = await ensureReadingArenaLevelNotCooling(username, majorLevel, minorLevel);
        if (!cooldownCheck.success) {
            return res.status(403).json({ success: false, error: cooldownCheck.error, cooldown: cooldownCheck.cooldown });
        }

        const sentences = Array.isArray(body.sentences) && body.sentences.length
            ? body.sentences
            : [passageRow.content];
        const userAnswers = Array.isArray(body.user_answers) ? body.user_answers : [];
        const aiJudges = Array.isArray(body.ai_judges) ? body.ai_judges : [];
        const aiComments = Array.isArray(body.ai_comments) ? body.ai_comments : [];
        const recommendedTranslations = Array.isArray(body.recommended_translations) ? body.recommended_translations : [];
        const aiReport = typeof body.ai_report === 'string' ? body.ai_report : '';
        const audioFilenames = normalizeReadingArenaStringArray(body.audio_filenames);
        const progressTrace = normalizeReadingArenaProgressTrace(body.progress_trace);
        const errorCount = extractReadingArenaErrorCount(body, aiComments, aiReport);
        const durationSeconds = extractReadingArenaDurationSeconds(body, progressTrace);
        const audioCount = extractReadingArenaAudioCount(body, audioFilenames);
        const answerPhase = typeof body.answer_phase === 'string' && body.answer_phase.trim() ? body.answer_phase.trim() : 'main';
        const answerType = typeof body.answer_type === 'string' && body.answer_type.trim() ? body.answer_type.trim() : 'read';
        const submitTime = typeof body.submit_time === 'string' && body.submit_time.trim() ? body.submit_time.trim() : nowISO();
        const createdAt = nowISO();
        const passageWordCount = Number(passageRow.word_count || countReadingArenaWords(passageRow.content));
        const accuracyRate = extractReadingArenaAccuracyRate(body, passageWordCount, errorCount);
        const powerAward = extractReadingArenaPowerAward(body, passageWordCount, accuracyRate);
        const powerMultiplier = extractReadingArenaPowerMultiplier(body, accuracyRate);
        const matchOutcome = extractReadingArenaMatchOutcome(body);

        const result = await dbRun(
            `INSERT INTO reading_arena_record (
                username, major_level, minor_level, passage_id, passage_word_count,
                error_count, accuracy_rate, duration_seconds, audio_count,
                power_award, power_multiplier, match_outcome,
                answer_phase, answer_type, sentences, user_answers, ai_judges,
                ai_comments, recommended_translations, ai_report, audio_filenames,
                progress_trace, submit_time, created_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)` ,
            [
                username,
                majorLevel,
                ...
                JSON.stringify(aiJudges),
                JSON.stringify(aiComments),
                JSON.stringify(recommendedTranslations),
                aiReport,
                JSON.stringify(audioFilenames),
                JSON.stringify(progressTrace),
                submitTime,
                createdAt
            ]
        );

        const savedRow = await dbGet(`SELECT * FROM reading_arena_record WHERE id = ? LIMIT 1`, [result.lastID]);
        res.json({
            success: true,
            id: result.lastID,
            record: savedRow ? serializeReadingArenaRecordRow(savedRow) : null
        });
    } catch (err) {
        console.error('保存朗读角斗场朗读记录失败:', err);
        res.status(500).json({ success: false, error: err.message || '保存朗读记录失败' });
    }
});

app.get('/api/reading-arena/daily-attempt-summary', async (req, res) => {
    try {
        const username = typeof req.query.username === 'string' ? req.query.username.trim() : '';
        if (!username) {
            return res.status(400).json({ success: false, error: '缺少有效的 username' });
        ...
        const summary = await getReadingArenaDailyAttemptSummary(username);
        res.json({ success: true, summary });
    } catch (err) {
        console.error('获取朗读角斗场每日次数失败:', err);
        res.status(500).json({ success: false, error: err.message || '获取每日次数失败' });
    }
});

app.post('/api/reading-arena/daily-attempt/consume', async (req, res) => {
    try {
        const body = req.body || {};
        const username = typeof body.username === 'string' ? body.username.trim() : '';
        const majorLevel = normalizePositiveInteger(body.majorLevel);
        ...
        if (!cooldownCheck.success) {
            const summary = await getReadingArenaDailyAttemptSummary(username);
            return res.status(403).json({ success: false, error: cooldownCheck.error, summary, cooldown: cooldownCheck.cooldown });
        }

        const consumeResult = await consumeReadingArenaDailyAttempt(username, majorLevel, minorLevel);
        if (!consumeResult.success) {
            return res.status(403).json({ success: false, error: consumeResult.error, summary: consumeResult.summary });
        }

        res.json({ success: true, summary: consumeResult.summary });
    } catch (err) {
        console.error('扣减朗读角斗场每日次数失败:', err);
        res.status(500).json({ success: false, error: err.message || '扣减每日次数失败' });
    }
});

app.get('/api/reading-arena/power-ranking', async (req, res) => {
    try {
        const limit = normalizePositiveInteger(req.query.limit) || 30;
        const ranking = await getReadingArenaPowerRanking(limit);
        res.json({ success: true, ranking });
    } catch (err) {
        console.error('获取朗读角斗场听说战力榜失败:', err);
        res.status(500).json({ success: false, error: err.message || '获取听说战力榜失败' });
    }
});

app.get('/api/reading-arena/ranking', async (req, res) => {
    try {
        const majorLevel = normalizePositiveInteger(req.query.majorLevel);
        const minorLevel = normalizePositiveInteger(req.query.minorLevel);
        ...
        const passageRow = await getReadingArenaPassageByMinorId(minorRow.id);
        const rows = await getReadingArenaRankingRows(majorLevel, minorLevel, limit);
        const ranking = (rows || []).map(buildReadingArenaRankedRecord);
        ...
        res.json({
            ...
            passage: passageRow ? {
                id: passageRow.id,
                wordCount: Number(passageRow.word_count || 0),
                updated_at: passageRow.updated_at,
                created_at: passageRow.created_at
            } : null,
            ranking
        });
    } catch (err) {
        console.error('获取朗读角斗场排行榜失败:', err);
        res.status(500).json({ success: false, error: err.message || '获取排行榜失败' });
    }
});

app.get('/api/reading-arena/match', async (req, res) => {
    try {
        const majorLevel = normalizePositiveInteger(req.query.majorLevel);
        const minorLevel = normalizePositiveInteger(req.query.minorLevel);
        const username = typeof req.query.username === 'string' ? req.query.username.trim() : '';
        ...
        const passageRow = await getReadingArenaPassageByMinorId(minorRow.id);
        if (!passageRow) {
            return res.status(404).json({ success: false, error: '该等级下暂无文章，暂时无法匹配对手' });
        }
        ...
        const rows = await getReadingArenaRankingRows(majorLevel, minorLevel);
        const ranking = (rows || []).map(buildReadingArenaRankedRecord);
        const match = chooseReadingArenaMatchOpponent(ranking, username);
        ...
        res.json({
            ...
            match: {
                ...
                candidateCount: Number(match.candidateCount || 0),
                referenceRecord: match.referenceRecord,
                soloEntry: !match.opponent,
                opponent: match.opponent
            }
        });
    } catch (err) {
        console.error('朗读角斗场匹配对手失败:', err);
        res.status(500).json({ success: false, error: err.message || '匹配对手失败' });
    }
});

app.get('/api/reading-arena/records', async (req, res) => {
    try {
        const hasMajorLevel = req.query.majorLevel != null && req.query.majorLevel !== '';
        const hasMinorLevel = req.query.minorLevel != null && req.query.minorLevel !== '';
        const majorLevel = hasMajorLevel ? normalizePositiveInteger(req.query.majorLevel) : null;
        const minorLevel = hasMinorLevel ? normalizePositiveInteger(req.query.minorLevel) : null;
        const username = typeof req.query.username === 'string' ? req.query.username.trim() : '';

        if (hasMajorLevel && !majorLevel) {
            return res.status(400).json({ success: false, error: 'majorLevel 必须是正整数' });
        }
        if (hasMinorLevel && !minorLevel) {
            return res.status(400).json({ success: false, error: 'minorLevel 必须是正整数' });
        }
        if (hasMinorLevel && !hasMajorLevel) {
            return res.status(400).json({ success: false, error: '查询 minorLevel 时必须同时提供 majorLevel' });
        }

        let sql = `SELECT * FROM reading_arena_record WHERE 1 = 1`;
        const params = [];

        if (majorLevel) {
            sql += ` AND major_level = ?`;
            params.push(majorLevel);
        }
        if (minorLevel) {
            sql += ` AND minor_level = ?`;
            params.push(minorLevel);
        }
        if (username) {
            sql += ` AND username = ?`;
            params.push(username);
        ...
        const rows = await dbAll(sql, params);
        const powerValue = username ? await getReadingArenaPowerValue(username) : null;

        res.json({
            success: true,
            powerValue,
            records: (rows || []).map(serializeReadingArenaRecordRow)
        });
    } catch (err) {
        console.error('获取朗读角斗场朗读记录列表失败:', err);
        res.status(500).json({ success: false, error: err.message || '获取朗读记录列表失败' });
    }
});

app.get('/api/reading-arena/record', async (req, res) => {
    try {
        const recordId = normalizePositiveInteger(req.query.id || req.query.recordId);
        if (!recordId) {
            return res.status(400).json({ success: false, error: '缺少有效的记录 id' });
        }

        const row = await dbGet(`SELECT * FROM reading_arena_record WHERE id = ? LIMIT 1`, [recordId]);
        if (!row) {
            return res.status(404).json({ success: false, error: '未找到对应的朗读记录' });
        }

        res.json({ success: true, record: serializeReadingArenaRecordRow(row) });
    } catch (err) {
        console.error('获取朗读角斗场朗读记录详情失败:', err);
        res.status(500).json({ success: false, error: err.message || '获取朗读记录详情失败' });
    }
});

app.post('/api/reading-arena/passage/delete', async (req, res) => {
    try {
        const majorLevel = req.body ? normalizePositiveInteger(req.body.majorLevel) : null;
        const minorLevel = req.body ? normalizePositiveInteger(req.body.minorLevel) : null;
        if (!majorLevel || !minorLevel) {
        ...
        const existingPassage = await getReadingArenaPassageByMinorId(minorRow.id);
        if (existingPassage) {
            await dbRun(
                `UPDATE reading_arena_record SET passage_id = NULL WHERE passage_id = ?`,
                [existingPassage.id]
            );
        }

        const result = await dbRun(
            `DELETE FROM reading_arena_passage WHERE minor_id = ?`,
            [minorRow.id]
        );

        if (!result.changes) {
            return res.status(404).json({ success: false, error: '该等级下暂无文章可删除' });
        }

        res.json({ success: true, deleted: result.changes || 0 });
    } catch (err) {
        console.error('删除朗读角斗场文章失败:', err);
        res.status(500).json({ success: false, error: err.message || '删除文章失败' });
    }
});

app.post('/api/reading-arena/minor-level/delete', async (req, res) => {
    try {
        const majorLevel = req.body ? normalizePositiveInteger(req.body.majorLevel) : null;
        const minorLevel = req.body ? normalizePositiveInteger(req.body.minorLevel) : null;
        if (!majorLevel || !minorLevel) {
        ...
        const minorRow = await getReadingArenaMinorByLevels(majorLevel, minorLevel);
        if (!minorRow) {
            return res.status(404).json({ success: false, error: '该小等级不存在' });
        }
        ...
        try {
            ...
            const deletedPassage = await dbRun(
                `DELETE FROM reading_arena_passage WHERE minor_id = ?`,
                [minorRow.id]
            );
            const deletedMinor = await dbRun(
                `DELETE FROM reading_arena_minor_level WHERE id = ?`,
                [minorRow.id]
            );
            await dbRun('COMMIT');
            ...
            res.json({
                success: true,
                deletedMinorLevels: deletedMinor.changes || 0,
                deletedPassages: deletedPassage.changes || 0
            });
        } catch (txErr) {
            await dbRun('ROLLBACK');
            throw txErr;
        ...
        console.error('删除朗读角斗场小等级失败:', err);
        res.status(500).json({ success: false, error: err.message || '删除小等级失败' });
    }
});

app.post('/api/reading-arena/major-level/delete', async (req, res) => {
    try {
        const majorLevel = req.body ? normalizePositiveInteger(req.body.majorLevel) : null;
        if (!majorLevel) {
            return res.status(400).json({ success: false, error: '缺少有效的 majorLevel' });
        ...
        const majorRow = await getReadingArenaMajorByLevel(majorLevel);
        if (!majorRow) {
            return res.status(404).json({ success: false, error: '该大等级不存在' });
        }

        await dbRun('BEGIN TRANSACTION');
        try {
            const passageRows = await dbAll(
                `SELECT id FROM reading_arena_passage
                 WHERE minor_id IN (
                     SELECT id FROM reading_arena_minor_level WHERE major_id = ?
                 )`,
                [majorRow.id]
            );
            const passageIds = (passageRows || []).map(row => Number(row.id)).filter(id => Number.isInteger(id) && id > 0);
            if (passageIds.length) {
                const placeholders = passageIds.map(() => '?').join(', ');
                await dbRun(
                    `UPDATE reading_arena_record
                     SET passage_id = NULL
                     WHERE passage_id IN (${placeholders})`,
                    passageIds
                );
            }
            const deletedPassages = await dbRun(
                `DELETE FROM reading_arena_passage
                 WHERE minor_id IN (
                     SELECT id FROM reading_arena_minor_level WHERE major_id = ?
                 )`,
                [majorRow.id]
            );
            const deletedMinors = await dbRun(
                `DELETE FROM reading_arena_minor_level WHERE major_id = ?`,
                [majorRow.id]
            );
            const deletedMajor = await dbRun(
                `DELETE FROM reading_arena_major_level WHERE id = ?`,
                [majorRow.id]
            );
            await dbRun('COMMIT');
            ...
            res.json({
                success: true,
                deletedMajorLevels: deletedMajor.changes || 0,
                deletedMinorLevels: deletedMinors.changes || 0,
                deletedPassages: deletedPassages.changes || 0
            });
        } catch (txErr) {
            await dbRun('ROLLBACK');
            throw txErr;
        ...
        console.error('删除朗读角斗场大等级失败:', err);
        res.status(500).json({ success: false, error: err.message || '删除大等级失败' });
    }
});
...
// ===== 文章管理开始 =====
// 新增：文章专用数据库
const articleDb = new sqlite3.Database('articledata.db');

// 初始化文章表
articleDb.serialize(() => {
    articleDb.run(`CREATE TABLE IF NOT EXISTS articles (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        title TEXT NOT NULL,
        content TEXT,
        updated_at DATETIME DEFAULT CURRENT_TIMESTAMP
...
// 1. 获取所有文章列表（用于文章展示页的左侧列表，以及管理页的列表）
app.get('/api/articles', (req, res) => {
    // 返回列表时带上更新时间，以便前端显示文章更新时间
    articleDb.all("SELECT id, title, updated_at FROM articles ORDER BY id DESC", (err, rows) => {
        if (err) {
...
// 2. 获取单篇文章详情（用于点击标题后加载右侧内容）
app.get('/api/articles/:id', (req, res) => {
    const id = req.params.id;
    articleDb.get("SELECT * FROM articles WHERE id = ?", [id], (err, row) => {
        if (err) {
            console.error("Fetch single article error:", err);
            return res.status(500).json({ error: err.message });
        }
        if (!row) {
...
app.post('/api/articles', (req, res) => {
    ...
    if (id) {
        // 如果有 ID，执行更新
        const sql = "UPDATE articles SET title = ?, content = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?";
        articleDb.run(sql, [title, content, id], function(err) {
            if (err) {
                console.error("Update article error:", err);
                return res.status(500).json({ error: err.message });
            }
            res.json({ success: true, id: this.changes });
        });
    } else {
        // 如果没有 ID，执行插入
        const sql = "INSERT INTO articles (title, content) VALUES (?, ?)";
        articleDb.run(sql, [title, content], function(err) {
            if (err) {
...
app.delete('/api/articles/:id', (req, res) => {
    const id = req.params.id;
    articleDb.run("DELETE FROM articles WHERE id = ?", [id], function(err) {
        if (err) {
